...
TrackDrivenAssociationAlg::TrackDrivenAssociationAlg()
    : m_minTrackAssociations(1), m_maxTrackAssociations(std::numeric_limits<unsigned int>::max()),
      m_chiToAttemptReclustering(-3.f), m_minChi2Improvement(1.f), m_coneCosineHalfAngle(0.9f),
      m_minFractionOfHitsInCone(0.2f), m_contactDistanceThreshold(2.5f), m_minContactLayers(5),
...
StatusCode TrackDrivenAssociationAlg::Run() {
  ...
  for (unsigned int i = 0; i < nClusters; ++i) {
    ...
    for (unsigned int j = 0; j < nClusters; ++j) {
      ...
      float bestFraction(FragmentRemovalHelper::GetFractionOfHitsInCone(this->GetPandora(), pDaughterCluster,
                                                                        pParentCluster, m_coneCosineHalfAngle));

      const TrackList& daughterTrackList(pDaughterCluster->GetAssociatedTrackList());

      for (TrackList::const_iterator iter = daughterTrackList.begin(), iterEnd = daughterTrackList.end();
           iter != iterEnd; ++iter) {
      ...
      for (TrackList::const_iterator iter = parentTrackList.begin(), iterEnd = parentTrackList.end(); iter != iterEnd;
        ...
        const float fraction(
            FragmentRemovalHelper::GetFractionOfHitsInCone(pDaughterCluster, *iter, m_coneCosineHalfAngle));
    ...
    for (StringVector::const_iterator clusteringIter = m_clusteringAlgorithms.begin(),
      ...
      PANDORA_RETURN_RESULT_IF(
          STATUS_CODE_SUCCESS, !=,
          PandoraContentApi::RunClusteringAlgorithm(*this, *clusteringIter, pReclusterList, reclusterListName));
    ...
    if (bestReclusterListName != originalClustersListName) {
      ...
      if (!shouldRecluster && (excessEnergy > m_minExcessEnergy)) {
        ...
        PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                                 PandoraContentApi::GetList(*this, bestReclusterListName, pBestReclusterList));

        for (ClusterList::const_iterator iter = pBestReclusterList->begin(), iterEnd = pBestReclusterList->end();
             iter != iterEnd; ++iter) {
...
StatusCode TrackDrivenAssociationAlg::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MinFractionOfHitsInCone", m_minFractionOfHitsInCone));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ContactDistanceThreshold", m_contactDistanceThreshold));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinContactLayers", m_minContactLayers));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinExcessEnergy", m_minExcessEnergy));
