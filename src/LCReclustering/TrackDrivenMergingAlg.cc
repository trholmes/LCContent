...
TrackDrivenMergingAlg::TrackDrivenMergingAlg()
    : m_minTrackAssociations(1), m_maxTrackAssociations(std::numeric_limits<unsigned int>::max()),
      m_chiToAttemptMerging(-2.5f), m_coarseDaughterChiCut(200.5f), m_coneCosineHalfAngle(0.9f),
      m_minConeFractionSingle(0.5f), m_minConeFractionMultiple(0.4f), m_maxLayerSeparationMultiple(5) {}
...
StatusCode TrackDrivenMergingAlg::Run() {
  ...
  for (unsigned int i = 0; i < nClusters; ++i) {
    ...
    if ((trackEnergySum < std::numeric_limits<float>::epsilon()) ||
        (hadronicEnergyResolution < std::numeric_limits<float>::epsilon()))
    ...
    for (unsigned int j = 0; j < nClusters; ++j) {
      ...
      const float newChi(
          (parentClusterEnergy + pDaughterCluster->GetTrackComparisonEnergy(this->GetPandora()) - trackEnergySum) /
          sigmaTrackEnergy);
      ...
      const float coneFraction(FragmentRemovalHelper::GetFractionOfHitsInCone(this->GetPandora(), pDaughterCluster,
      ...
      if (coneFraction > m_minConeFractionMultiple) {
        if ((daughterInnerLayer < parentOuterLayer) ||
            (daughterInnerLayer - parentOuterLayer < m_maxLayerSeparationMultiple)) {
    ...
    if ((highestConeFraction > m_minConeFractionSingle) && (NULL != pBestDaughterCluster) &&
        (MAX_INDEX != bestDaughterClusterIndex)) {
      clusterVector[bestDaughterClusterIndex] = NULL;
      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                               PandoraContentApi::MergeAndDeleteClusters(*this, pParentCluster, pBestDaughterCluster));
      continue;
    }

    // If single merge unsuccessful, consider multiple merges: work through the cone fraction list, starting with
    // highest fractions
    ...
    for (ClusterConeFractionList::const_iterator iter = clusterConeFractionList.begin(),
                                                 iterEnd = clusterConeFractionList.end();
         iter != iterEnd; ++iter) {
    ...
    for (ClusterConeFractionList::const_iterator iter = daughterClusters.begin(), iterEnd = daughterClusters.end();
         iter != iterEnd; ++iter) {
      clusterVector[iter->GetClusterIndex()] = NULL;
      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                               PandoraContentApi::MergeAndDeleteClusters(*this, pParentCluster, iter->GetCluster()));
    }

    // ATTN: haven't implemented the "dubious partial merge" present in old Pandora. This would merge daughter cluster
    // with highest fraction above 0.75, in situation where std::fabs(newChi) > 2.5 AND (newChi > 2.5 OR magnitude of
    // chi is increasing).
  }
...
StatusCode TrackDrivenMergingAlg::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ChiToAttemptMerging", m_chiToAttemptMerging));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "CoarseDaughterChiCut", m_coarseDaughterChiCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ConeCosineHalfAngle", m_coneCosineHalfAngle));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinConeFractionSingle", m_minConeFractionSingle));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MinConeFractionMultiple", m_minConeFractionMultiple));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MaxLayerSeparationMultiple", m_maxLayerSeparationMultiple));
