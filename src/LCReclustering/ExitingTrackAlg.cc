...
ExitingTrackAlg::ExitingTrackAlg()
    : m_minTrackAssociations(1), m_maxTrackAssociations(1), m_chiToAttemptReclustering(2.f), m_minChi2Improvement(1.f),
  ...
  for (ClusterVector::iterator iter = clusterVector.begin(), iterEnd = clusterVector.end(); iter != iterEnd; ++iter) {
    ...
    for (StringVector::const_iterator clusteringIter = m_clusteringAlgorithms.begin(),
      ...
      if ((bestReclusterChi2 - reclusterChi2 > m_minChi2Improvement) &&
          (reclusterResult.GetChi() < m_chiToAttemptReclustering)) {
