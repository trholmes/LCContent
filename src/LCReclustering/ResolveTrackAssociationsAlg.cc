...
ResolveTrackAssociationsAlg::ResolveTrackAssociationsAlg()
    : m_minTrackAssociations(1), m_maxTrackAssociations(std::numeric_limits<unsigned int>::max()),
      m_chiToAttemptReclustering(-3.f), m_minChi2Improvement(1.f), m_coneCosineHalfAngle(0.9f), m_minConeFraction(0.2f),
      m_minClusterEnergyForTrackAssociation(0.1f), m_chi2ForAutomaticClusterSelection(1.f),
      m_usingOrderedAlgorithms(false), m_bestChi2ForReclusterHalt(4.f), m_currentChi2ForReclusterHalt(16.f),
      m_shouldUseBestGuessCandidates(true), m_shouldUseForcedClustering(false), m_minChiForForcedClustering(4.f),
      m_minForcedChi2Improvement(9.f), m_maxForcedChi2(36.f) {}
  ...
  for (unsigned int i = 0; i < nClusters; ++i) {
    ...
    for (unsigned int j = 0; j < nClusters; ++j) {
      ...
      if ((NULL == pDaughterCluster) || (pParentCluster == pDaughterCluster) ||
          (!pDaughterCluster->GetAssociatedTrackList().empty()))
        continue;

      if (FragmentRemovalHelper::GetFractionOfHitsInCone(this->GetPandora(), pDaughterCluster, pParentCluster,
                                                         m_coneCosineHalfAngle) > m_minConeFraction) {
    ...
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                             PandoraContentApi::InitializeReclustering(*this, reclusterTrackList, reclusterClusterList,
    ...
    for (StringVector::const_iterator clusteringIter = m_clusteringAlgorithms.begin(),
                                      clusteringIterEnd = m_clusteringAlgorithms.end();
      ...
      PANDORA_RETURN_RESULT_IF(
          STATUS_CODE_SUCCESS, !=,
          PandoraContentApi::RunClusteringAlgorithm(*this, *clusteringIter, pReclusterList, reclustersListName));
      ...
      if (STATUS_CODE_SUCCESS !=
          ReclusterHelper::ExtractReclusterResults(this->GetPandora(), pReclusterList, reclusterResult))
        continue;
      ...
      // If no ideal candidate is found, store a best guess candidate for future modification
        ...
        if ((reclusterResult.GetNExcessTrackAssociations() > 0) && (reclusterResult.GetChi() > 0) &&
            (reclusterResult.GetChi() < bestGuessChi)) {
      ...
      if (m_usingOrderedAlgorithms && (bestReclusterChi2 < m_bestChi2ForReclusterHalt) &&
          (reclusterChi2 > m_currentChi2ForReclusterHalt))
        break;
    }

    // If no ideal candidate constructed, can choose to use best guess candidates, which could be split by later
    // algorithms
    ...
    if (m_shouldUseForcedClustering) {
      if ((bestReclusterListName == originalClustersListName) || (bestReclusterChi > m_minChiForForcedClustering)) {
        ...
        PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                                 PandoraContentApi::RunClusteringAlgorithm(*this, m_forcedClusteringAlgorithmName,
        ...
        if (STATUS_CODE_SUCCESS ==
            ReclusterHelper::ExtractReclusterResults(this->GetPandora(), pForcedClusterList, forcedClusterResult)) {
    ...
    if (bestReclusterListName != originalClustersListName) {
      for (UIntVector::const_iterator iter = originalClusterIndices.begin(), iterEnd = originalClusterIndices.end();
           iter != iterEnd; ++iter)
...
StatusCode ResolveTrackAssociationsAlg::ReadSettings(const TiXmlHandle xmlHandle) {
  PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=,
      XmlHelper::ProcessAlgorithmList(*this, xmlHandle, "clusteringAlgorithms", m_clusteringAlgorithms));
  ...
  PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=,
      XmlHelper::ProcessAlgorithm(*this, xmlHandle, "TrackClusterAssociation", m_trackClusterAssociationAlgName));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinTrackAssociations", m_minTrackAssociations));

  if (m_minTrackAssociations < 1)
    return STATUS_CODE_INVALID_PARAMETER;

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxTrackAssociations", m_maxTrackAssociations));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ChiToAttemptReclustering", m_chiToAttemptReclustering));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinChi2Improvement", m_minChi2Improvement));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MinClusterEnergyForTrackAssociation", m_minClusterEnergyForTrackAssociation));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "Chi2ForAutomaticClusterSelection", m_chi2ForAutomaticClusterSelection));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "UsingOrderedAlgorithms", m_usingOrderedAlgorithms));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "BestChi2ForReclusterHalt", m_bestChi2ForReclusterHalt));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "CurrentChi2ForReclusterHalt", m_currentChi2ForReclusterHalt));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ShouldUseBestGuessCandidates", m_shouldUseBestGuessCandidates));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ShouldUseForcedClustering", m_shouldUseForcedClustering));

  if (m_shouldUseForcedClustering) {
    PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=,
        XmlHelper::ProcessAlgorithm(*this, xmlHandle, "ForcedClustering", m_forcedClusteringAlgorithmName));
  }

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MinChiForForcedClustering", m_minChiForForcedClustering));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MinForcedChi2Improvement", m_minForcedChi2Improvement));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxForcedChi2", m_maxForcedChi2));
