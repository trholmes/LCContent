...
SplitMergedClustersAlg::SplitMergedClustersAlg()
    : m_minTrackAssociations(1), m_maxTrackAssociations(std::numeric_limits<unsigned int>::max()),
      m_chiToAttemptReclustering(3.f), m_chiToForceReclustering(4.f), m_minChi2Improvement(1.f),
      ...
      m_shouldUseForcedClustering(false), m_minChiForForcedClustering(4.f), m_minForcedChi2Improvement(9.f),
...
StatusCode SplitMergedClustersAlg::Run() {
  ...
  for (ClusterVector::iterator iter = clusterVector.begin(), iterEnd = clusterVector.end(); iter != iterEnd; ++iter) {
    ...
    const float chi2Cut((chi > m_chiToForceReclustering) ? m_chiToForceReclustering * m_chiToForceReclustering
                                                         : chi2 - m_minChi2Improvement);
...
StatusCode SplitMergedClustersAlg::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ChiToForceReclustering", m_chiToForceReclustering));
