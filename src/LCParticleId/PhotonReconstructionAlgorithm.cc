...
PhotonReconstructionAlgorithm::PhotonReconstructionAlgorithm()
    : m_replaceCurrentClusterList(false), m_shouldDeleteNonPhotonClusters(true), m_minClusterEnergy(0.f),
      m_minPeakEnergy(0.f), m_maxPeakRms(5.f), m_maxRmsRatio(3.f), m_maxLongProfileStart(10.f),
      m_maxLongProfileDiscrepancy(0.8f), m_maxSearchLayer(9), m_parallelDistanceCut(100.f),
      m_minTrackClusterCosAngle(0.f), m_minDistanceToTrackDivisionCut(3.f), m_transProfileEcalOnly(true),
      m_transProfileMaxLayer(0), m_minDistanceToTrackCutLow(2.f), m_minDistanceToTrackCutHigh(200.f),
      m_energyCutForPid1(0.2f), m_pidCut1(1.f), m_energyCutForPid2(0.5f), m_pidCut2(0.6f), m_pidCut3(0.4f),
      // histogram related initialisation
      m_shouldMakePdfHistograms(false), m_shouldDrawPdfHistograms(false), m_nEnergyBins(0) {}
...
PhotonReconstructionAlgorithm::~PhotonReconstructionAlgorithm() {
  ...
  for (PDFVarLikelihoodPDFMap::iterator iter = m_pdfVarLikelihoodPDFMap.begin(),
                                        iterEnd = m_pdfVarLikelihoodPDFMap.end();
       iter != iterEnd; ++iter) {
...
StatusCode PhotonReconstructionAlgorithm::Run() {
  // ATTN Implicit assumption that individual physical layers in the ECAL will always correspond to individual pseudo
  // layers Also ECAL BARREL has same layer as ECAL ENDCAP and ecal is the first inner detector
  ...
  for (ClusterVector::const_iterator iter = clusterVector.begin(), iterEnd = clusterVector.end(); iter != iterEnd;
    ...
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                             this->GetMinDistanceToTrack(pCluster, trackVector, minDistance, pMinTrack));
    ...
    if (pMinTrack && minDistance < m_minDistanceToTrackDivisionCut) {
      ...
      PANDORA_RETURN_RESULT_IF(
          STATUS_CODE_SUCCESS, !=,
          this->GetTrackClusterShowerList(pCluster, pMinTrack, trackVector, showersPhoton, showersCharged));
      fromTrack = true;
    } else {
...
StatusCode PhotonReconstructionAlgorithm::InitialiseInputClusterListName(std::string& inputClusterListName) const {
  ...
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           PandoraContentApi::GetCurrentList(*this, pInputClusterList, inputClusterListName));
  return STATUS_CODE_SUCCESS;
}
...
StatusCode PhotonReconstructionAlgorithm::CreateClustersOfInterest(ClusterVector& clusterVector) const {
  ...
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           PandoraContentApi::RunClusteringAlgorithm(*this, m_photonClusteringAlgName,
                                                                     pPhotonClusterList, photonClusterListName));

  if (pPhotonClusterList->empty()) {
    std::cout << "PhotonReconstructionAlgorithm::CreateRegionsOfInterests - no photon candidates available, no regions "
...
StatusCode
PhotonReconstructionAlgorithm::GetTracklessClusterShowerList(const Cluster* const pCluster,
                                                             ShowerProfilePlugin::ShowerPeakList& showersPhoton) const {
...
StatusCode PhotonReconstructionAlgorithm::GetTrackClusterShowerList(
    const Cluster* const pCluster, const Track* const pMinTrack, const TrackVector trackVector,
    ShowerProfilePlugin::ShowerPeakList& showersPhoton, ShowerProfilePlugin::ShowerPeakList& showersCharged) const {
  const ShowerProfilePlugin* const pShowerProfilePlugin(PandoraContentApi::GetPlugins(*this)->GetShowerProfilePlugin());
  pShowerProfilePlugin->CalculateTrackBasedTransverseProfile(pCluster, m_transProfileMaxLayer, pMinTrack, trackVector,
                                                             showersPhoton, showersCharged);
  return STATUS_CODE_SUCCESS;
}

//------------------------------------------------------------------------------------------------------------------------------------------

StatusCode PhotonReconstructionAlgorithm::CreatePhotons(const Cluster* const pCluster,
                                                        const ShowerProfilePlugin::ShowerPeakList& showersPhoton,
                                                        const bool isFromTrack) const {
  ...
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           this->InitialiseFragmentation(pCluster, originalClusterListName, peakClusterListName));
  bool usedCluster(false);

  try {
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                             this->CreateClustersAndSetPhotonID(showersPhoton, pCluster->GetElectromagneticEnergy(),
                                                                usedCluster, isFromTrack));
  } catch (const StatusCodeException&) {
    usedCluster = false;
    std::cout
        << "PhotonReconstructionAlgorithm::CreatePhotons: StatusCodeException caught during fragmentation process."
        << std::endl;
  }

  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           this->EndFragmentation(usedCluster, originalClusterListName, peakClusterListName));
...
StatusCode PhotonReconstructionAlgorithm::InitialiseFragmentation(const Cluster* const pCluster,
                                                                  std::string& originalClusterListName,
                                                                  std::string& peakClusterListName) const {
  const ClusterList clusterList(1, pCluster);
  PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=,
      PandoraContentApi::InitializeFragmentation(*this, clusterList, originalClusterListName, peakClusterListName));
  return STATUS_CODE_SUCCESS;
}

//------------------------------------------------------------------------------------------------------------------------------------------

StatusCode PhotonReconstructionAlgorithm::EndFragmentation(const bool usedCluster,
                                                           const std::string& originalClusterListName,
                                                           const std::string& peakClusterListName) const {
...
StatusCode
PhotonReconstructionAlgorithm::CreateClustersAndSetPhotonID(const ShowerProfilePlugin::ShowerPeakList& showersPhoton,
                                                            const float wholeClusuterEnergy, bool& usedCluster,
                                                            const bool isFromTrack) const {
  for (unsigned int iPeak = 0, iPeakEnd = showersPhoton.size(); iPeak < iPeakEnd; ++iPeak) {
    ...
    PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=,
        this->CheckAndSetPhotonID(showerPeak, pPeakCluster, wholeClusuterEnergy, isPhoton, isFromTrack));
    if (isPhoton) {
      usedCluster = true;
    } else {
...
StatusCode PhotonReconstructionAlgorithm::CreateCluster(const ShowerProfilePlugin::ShowerPeak& showerPeak,
                                                        const Cluster*& pPeakCluster) const {
  PandoraContentApi::Cluster::Parameters parameters;
  parameters.m_caloHitList = showerPeak.GetPeakCaloHitList();
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           PandoraContentApi::Cluster::Create(*this, parameters, pPeakCluster));
  return STATUS_CODE_SUCCESS;
}

//------------------------------------------------------------------------------------------------------------------------------------------

StatusCode PhotonReconstructionAlgorithm::CheckAndSetPhotonID(const ShowerProfilePlugin::ShowerPeak& showerPeak,
                                                              const Cluster* const pPeakCluster,
                                                              const float wholeClusterEnergy, bool& isPhoton,
                                                              const bool isFromTrack) const {
  PDFVarFloatMap pdfVarFloatMap;
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           this->CalculateForPhotonID(showerPeak, pPeakCluster, wholeClusterEnergy, pdfVarFloatMap));
...
StatusCode PhotonReconstructionAlgorithm::CalculateForPhotonID(const ShowerProfilePlugin::ShowerPeak& showerPeak,
                                                               const Cluster* const pPeakCluster,
                                                               const float wholeClusterEnergy,
                                                               PDFVarFloatMap& pdfVarFloatMap) const {
  ...
  PandoraContentApi::GetPlugins(*this)->GetShowerProfilePlugin()->CalculateLongitudinalProfile(
      pPeakCluster, profileStart, profileDiscrepancy);
  ...
  float minDistance(std::numeric_limits<float>::max());
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           this->GetMinDistanceToTrack(pPeakCluster, trackVector, minDistance, pMinTrack));
...
bool PhotonReconstructionAlgorithm::IsPhoton(const pandora::Cluster* const pPeakCluster,
                                             const PDFVarFloatMap& pdfVarFloatMap, const bool isFromTrack) const {
...
StatusCode PhotonReconstructionAlgorithm::SetPhotonID(const Cluster* const pPeakCluster) const {
  ...
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           PandoraContentApi::Cluster::AlterMetadata(*this, pPeakCluster, metadata));
  return STATUS_CODE_SUCCESS;
}

//------------------------------------------------------------------------------------------------------------------------------------------

bool PhotonReconstructionAlgorithm::PassPhotonQualityCut(const float clusterEnergy,
                                                         const PDFVarFloatMap& pdfVarFloatMap) const {
  return (clusterEnergy > m_minPeakEnergy && pdfVarFloatMap.find(PEAKRMS)->second < m_maxPeakRms &&
...
float PhotonReconstructionAlgorithm::GetPIDForPhotonID(const float clusterEnergy,
                                                       const PDFVarFloatMap& pdfVarFloatMap) const {
  double yes(1.f), no(1.f);
  const unsigned int energyBin(this->GetEnergyBin(clusterEnergy));

  for (PDFVarLikelihoodPDFMap::const_iterator iter = m_pdfVarLikelihoodPDFMap.begin(),
                                              iterEnd = m_pdfVarLikelihoodPDFMap.end();
       iter != iterEnd; ++iter) {
    ...
    no *=
        this->GetHistogramContent(likelihoodPDFObject.m_pBackgroundPDF[energyBin], pdfVarFloatMap.find(pdfVar)->second);
  }
  const double yesWeighted(yes * m_nSignalEvents[energyBin]), noWeighted(no * m_nBackgroundEvents[energyBin]);
  return ((yesWeighted + noWeighted) > std::numeric_limits<double>::epsilon()
              ? static_cast<float>(yesWeighted / (yesWeighted + noWeighted))
              : 0.f);
}

//------------------------------------------------------------------------------------------------------------------------------------------

bool PhotonReconstructionAlgorithm::PassPhotonPIDCut(const float pid, const float clusterEnergy,
                                                     const bool /*isFromTrack*/) const {
...
StatusCode PhotonReconstructionAlgorithm::RunNestedFragmentRemovalAlg() const {
  ...
  if (!m_fragmentMergingAlgName.empty()) {
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                             PandoraContentApi::RunDaughterAlgorithm(*this, m_fragmentMergingAlgName));
  }
  return STATUS_CODE_SUCCESS;
}
...
StatusCode PhotonReconstructionAlgorithm::ReplaceInputClusterList(const std::string& inputClusterListName) const {
  ...
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           PandoraContentApi::ReplaceCurrentList<Cluster>(*this, replacementListName));
  return STATUS_CODE_SUCCESS;
}

//------------------------------------------------------------------------------------------------------------------------------------------

StatusCode PhotonReconstructionAlgorithm::GetMinDistanceToTrack(const pandora::Cluster* const pCluster,
                                                                const pandora::TrackVector& trackVector,
                                                                float& minDistance,
                                                                const pandora::Track*& pMinTrack) const {
  ...
  for (TrackVector::const_iterator trackIter = trackVector.begin(), trackIterEnd = trackVector.end();
    ...
    if (STATUS_CODE_SUCCESS ==
        lc_content::ClusterHelper::GetTrackClusterDistance(pTrack, pCluster, m_maxSearchLayer, m_parallelDistanceCut,
...
StatusCode PhotonReconstructionAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=,
      XmlHelper::ProcessAlgorithm(*this, xmlHandle, "PhotonClusterFormation", m_photonClusteringAlgName));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ProcessAlgorithm(*this, xmlHandle, "PhotonFragmentMerging", m_fragmentMergingAlgName));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ShouldDeleteNonPhotonClusters", m_shouldDeleteNonPhotonClusters));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinClusterEnergy", m_minClusterEnergy));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinPeakEnergy", m_minPeakEnergy));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxPeakRms", m_maxPeakRms));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxLongProfileStart", m_maxLongProfileStart));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MaxLongProfileDiscrepancy", m_maxLongProfileDiscrepancy));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MinDistanceToTrackDivisionCut", m_minDistanceToTrackDivisionCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "TransProfileEcalOnly", m_transProfileEcalOnly));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "TransProfileMaxLayer", m_transProfileMaxLayer));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MinDistanceToTrackCutLow", m_minDistanceToTrackCutLow));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MinDistanceToTrackCutHigh", m_minDistanceToTrackCutHigh));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "EnergyCutForPid1", m_energyCutForPid1));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "PidCut1", m_pidCut1));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "EnergyCutForPid2", m_energyCutForPid2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "PidCut2", m_pidCut2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "PidCut3", m_pidCut3));
...
StatusCode PhotonReconstructionAlgorithm::ReadHistogramSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ShouldMakePdfHistograms", m_shouldMakePdfHistograms));
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ShouldDrawPdfHistograms", m_shouldDrawPdfHistograms));
...
StatusCode PhotonReconstructionAlgorithm::InitialiseHistogramWriting(const pandora::TiXmlHandle xmlHandle) {
  ...
  if (0 == m_nEnergyBins || 10000 < m_nEnergyBins) {
    std::cout << "PhotonReconstructionAlgorithm::InitialiseHistogramWriting - Invalid number PDF energy bins specified."
              << std::endl;
    return STATUS_CODE_INVALID_PARAMETER;
  }

  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           this->GetEnergyBinLowerEdges(xmlHandle, "EnergyBinLowerEdges", m_energyBinLowerEdges));

  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           this->FillPDFVarLikelihoodPDFMapParameters(xmlHandle, PEAKRMS, "PeakRmsNBins", 50,
                                                                      "PeakRmsLowValue", 0.f, "PeakRmsHighValue", 5.f));
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           this->FillPDFVarLikelihoodPDFMapParameters(xmlHandle, RMSXYRATIO, "RmsRatioNBins", 30,
                                                                      "RmsRatioLowValue", 1.f, "RmsRatioHighValue",
                                                                      3.f));
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           this->FillPDFVarLikelihoodPDFMapParameters(
                               xmlHandle, LONGPROFILESTART, "LongProfileStartNBins", 11, "LongProfileStartLowValue",
                               -0.5f, "LongProfileStartHighValue", 10.5f));
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           this->FillPDFVarLikelihoodPDFMapParameters(
                               xmlHandle, LONGPROFILEDISCREPANCY, "LongProfileDiscrepancyNBins", 42,
                               "LongProfileDiscrepancyLowValue", -0.02f, "LongProfileDiscrepancyHighValue", 0.82f));
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           this->FillPDFVarLikelihoodPDFMapParameters(
                               xmlHandle, PEAKENERGYFRACTION, "PeakEnergyFractionNBins", 52,
                               "PeakEnergyFractionLowValue", -0.02f, "PeakEnergyFractionHighValue", 1.02f));
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           this->FillPDFVarLikelihoodPDFMapParameters(
                               xmlHandle, MINDISTANCETOTRACK, "MinDistanceToTrackNBins", 40,
  ...
  for (PDFVarLikelihoodPDFMap::iterator iter = m_pdfVarLikelihoodPDFMap.begin(),
    ...
    for (unsigned int energyBin = 0; energyBin < m_nEnergyBins; ++energyBin) {
      likelihoodPDFObject.m_pSignalPDF[energyBin] =
          new Histogram(likelihoodPDFObject.m_nBins, likelihoodPDFObject.m_lowValue, likelihoodPDFObject.m_highValue);
      likelihoodPDFObject.m_pBackgroundPDF[energyBin] =
          new Histogram(likelihoodPDFObject.m_nBins, likelihoodPDFObject.m_lowValue, likelihoodPDFObject.m_highValue);
    }
  }
...
StatusCode PhotonReconstructionAlgorithm::InitialiseHistogramReading() {
  ...
  // ATTN Sanity check: m_nEnergyBins should have a reasonable value
  if (0 == m_nEnergyBins || 10000 < m_nEnergyBins) {
    std::cout << "PhotonReconstructionAlgorithm::InitialiseHistogramReading - Invalid number PDF energy bins specified."
              << std::endl;
    return STATUS_CODE_INVALID_PARAMETER;
  }

  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           this->GetEnergyBinLowerEdges(pdfXmlHandle, "EnergyBinLowerEdges", m_energyBinLowerEdges));
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           this->GetNSignalBackgroundEvts(pdfXmlHandle, "NSignalEvents", "NBackgroundEvents",
                                                          m_nSignalEvents, m_nBackgroundEvents));
  ...
  for (PDFVarLikelihoodPDFMap::iterator iter = m_pdfVarLikelihoodPDFMap.begin(),
    ...
    for (unsigned int energyBin = 0; energyBin < m_nEnergyBins; ++energyBin) {
      likelihoodPDFObject.m_pSignalPDF[energyBin] =
          new Histogram(&pdfXmlHandle, "PhotonSig" + likelihoodPDFObject.m_pdfVarName + "_" + TypeToString(energyBin));
      likelihoodPDFObject.m_pBackgroundPDF[energyBin] =
          new Histogram(&pdfXmlHandle, "PhotonBkg" + likelihoodPDFObject.m_pdfVarName + "_" + TypeToString(energyBin));
    }
  }
  return STATUS_CODE_SUCCESS;
}
...
StatusCode PhotonReconstructionAlgorithm::InitialisePDFVarLikelihoodPDFObjectMap() {
  if (false ==
      m_pdfVarLikelihoodPDFMap.insert(PDFVarLikelihoodPDFMap::value_type(PEAKRMS, LikelihoodPDFObject("PeakRms")))
          .second)
    return STATUS_CODE_FAILURE;
  if (false ==
      m_pdfVarLikelihoodPDFMap.insert(PDFVarLikelihoodPDFMap::value_type(RMSXYRATIO, LikelihoodPDFObject("RmsRatio")))
          .second)
    return STATUS_CODE_FAILURE;
  if (false ==
      m_pdfVarLikelihoodPDFMap
          .insert(PDFVarLikelihoodPDFMap::value_type(LONGPROFILESTART, LikelihoodPDFObject("LongProfileStart")))
          .second)
    return STATUS_CODE_FAILURE;
  if (false == m_pdfVarLikelihoodPDFMap
                   .insert(PDFVarLikelihoodPDFMap::value_type(LONGPROFILEDISCREPANCY,
                                                              LikelihoodPDFObject("LongProfileDiscrepancy")))
                   .second)
    return STATUS_CODE_FAILURE;
  if (false ==
      m_pdfVarLikelihoodPDFMap
          .insert(PDFVarLikelihoodPDFMap::value_type(PEAKENERGYFRACTION, LikelihoodPDFObject("PeakEnergyFraction")))
          .second)
    return STATUS_CODE_FAILURE;
  if (false ==
      m_pdfVarLikelihoodPDFMap
          .insert(PDFVarLikelihoodPDFMap::value_type(MINDISTANCETOTRACK, LikelihoodPDFObject("MinDistanceToTrack")))
          .second)
    return STATUS_CODE_FAILURE;
  return STATUS_CODE_SUCCESS;
}
...
StatusCode PhotonReconstructionAlgorithm::GetNEnergyBins(const pandora::TiXmlHandle xmlHandle,
                                                         const std::string& nEnergyBinsStr,
                                                         unsigned int& nEnergyBins) const {
...
StatusCode PhotonReconstructionAlgorithm::GetEnergyBinLowerEdges(const TiXmlHandle xmlHandle,
                                                                 const std::string& energyBinLowerEdgesStr,
                                                                 pandora::FloatVector& energyBinLowerEdges) const {
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           XmlHelper::ReadVectorOfValues(xmlHandle, energyBinLowerEdgesStr, energyBinLowerEdges));
...
StatusCode PhotonReconstructionAlgorithm::GetNSignalBackgroundEvts(const pandora::TiXmlHandle xmlHandle,
                                                                   const std::string& nSignalEventsStr,
                                                                   const std::string& nBackgroundEventsStr,
                                                                   pandora::IntVector& nSignalEvents,
                                                                   pandora::IntVector& nBackgroundEvents) const {
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           XmlHelper::ReadVectorOfValues(xmlHandle, nSignalEventsStr, nSignalEvents));
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           XmlHelper::ReadVectorOfValues(xmlHandle, nBackgroundEventsStr, nBackgroundEvents));
  return STATUS_CODE_SUCCESS;
}

//------------------------------------------------------------------------------------------------------------------------------------------

StatusCode PhotonReconstructionAlgorithm::FillPDFVarLikelihoodPDFMapParameters(
    const TiXmlHandle xmlHandle, const PDFVar pdfVar, const std::string& nBinStr, const int nBinDefault,
    const std::string& lowValueStr, const float lowValueDefault, const std::string& highValueStr,
    const float highValueDefault) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, nBinStr, likelihoodPDFObject.m_nBins));

  likelihoodPDFObject.m_lowValue = lowValueDefault;
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, lowValueStr, likelihoodPDFObject.m_lowValue));

  likelihoodPDFObject.m_highValue = highValueDefault;
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, highValueStr, likelihoodPDFObject.m_highValue));
...
unsigned int PhotonReconstructionAlgorithm::GetEnergyBin(const float energy) const {
  FloatVector::const_iterator binEdgesIter =
      std::upper_bound(m_energyBinLowerEdges.begin(), m_energyBinLowerEdges.end(), energy);
...
pandora::StatusCode
PhotonReconstructionAlgorithm::CreatePhotonsForTraining(const Cluster* const pCluster,
                                                        const ShowerProfilePlugin::ShowerPeakList& showersPhoton) {
  // ATTN showersCharged or showersPhoton could empty
  if (showersPhoton.empty()) {
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, this->DeleteCluster(pCluster));
  ...
  try {
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                             this->CreateClustersAndTrainPhotonID(showersPhoton, pCluster->GetElectromagneticEnergy()));
  } catch (const StatusCodeException&) {
    std::cout << "PhotonReconstructionAlgorithm::CreatePhotonsForTraining: StatusCodeException caught during "
  ...
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           this->EndFragmentation(dummyBoolean, originalClusterListName, peakClusterListName));
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, this->DeleteCluster(pCluster));
...
StatusCode
PhotonReconstructionAlgorithm::CreateClustersAndTrainPhotonID(const ShowerProfilePlugin::ShowerPeakList& showersPhoton,
                                                              const float wholeClusuterEnergy) {
  for (unsigned int iPeak = 0, iPeakEnd = showersPhoton.size(); iPeak < iPeakEnd; ++iPeak) {
    const ShowerProfilePlugin::ShowerPeak& showerPeak(showersPhoton[iPeak]);
    const Cluster* pPeakCluster = NULL;
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, this->CreateCluster(showerPeak, pPeakCluster));
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                             this->TrainPhotonID(showerPeak, pPeakCluster, wholeClusuterEnergy));
  }
  return STATUS_CODE_SUCCESS;
}
...
StatusCode PhotonReconstructionAlgorithm::TrainPhotonID(const ShowerProfilePlugin::ShowerPeak& showerPeak,
                                                        const Cluster* const pPeakCluster,
                                                        const float wholeClusuterEnergy) {
  PDFVarFloatMap pdfVarFloatMap;
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           this->CalculateForPhotonID(showerPeak, pPeakCluster, wholeClusuterEnergy, pdfVarFloatMap));
...
StatusCode PhotonReconstructionAlgorithm::FillPdfHistograms(const pandora::Cluster* const pCluster,
                                                            const PDFVarFloatMap& pdfVarFloatMap) {
...
void PhotonReconstructionAlgorithm::NormalizeHistogram(Histogram* const pHistogram) const {
  ...
  const float cumulativeSum(
      pHistogram->GetCumulativeSum(pHistogram->GetUnderflowBinNumber(), pHistogram->GetOverflowBinNumber()));
...
void PhotonReconstructionAlgorithm::WriteString(TiXmlDocument& xmlDocument, const std::string nameStr,
...
void PhotonReconstructionAlgorithm::NormalizeAndWriteHistograms() {
  ...
  for (PDFVarLikelihoodPDFMap::iterator iter = m_pdfVarLikelihoodPDFMap.begin(),
    ...
    LikelihoodPDFObject& likelihoodPDFObject((*iter).second);
    for (unsigned int energyBin = 0; energyBin < m_nEnergyBins; ++energyBin) {
      likelihoodPDFObject.m_pSignalPDF[energyBin]->WriteToXml(
          &xmlDocument, "PhotonSig" + likelihoodPDFObject.m_pdfVarName + "_" + TypeToString(energyBin));
      likelihoodPDFObject.m_pBackgroundPDF[energyBin]->WriteToXml(
          &xmlDocument, "PhotonBkg" + likelihoodPDFObject.m_pdfVarName + "_" + TypeToString(energyBin));
    }
  }
...
void PhotonReconstructionAlgorithm::DrawHistograms() const {
  for (unsigned int energyBin = 0; energyBin < m_nEnergyBins; ++energyBin) {
    ...
    for (PDFVarLikelihoodPDFMap::const_iterator iter = m_pdfVarLikelihoodPDFMap.begin(),
         ...
         iter != iterEnd; ++iter) {
      const LikelihoodPDFObject& likelihoodPDFObject((*iter).second);
      std::cout << "PhotonReconstructionAlgorithm: " << likelihoodPDFObject.m_pdfVarName << ", Signal, Background "
                << std::endl;
      ...
      try {
        ...
        PANDORA_MONITORING_API(
            DrawPandoraHistogram(this->GetPandora(), *likelihoodPDFObject.m_pBackgroundPDF[energyBin]));
      } catch (const StatusCodeException&) {
        std::cout << "PhotonReconstructionAlgorithm: unable to draw histograms for variable "
                  << likelihoodPDFObject.m_pdfVarName << std::endl;
      }
    }
  }
}
