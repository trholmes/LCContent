...
PhotonSplittingAlgorithm::PhotonSplittingAlgorithm()
    : m_maxSearchLayer(9), m_parallelDistanceCut(100.f), m_minTrackClusterCosAngle(0.f),
      m_maxDistanceToTrackCut(20000.f), m_transProfileEcalOnly(true), m_transProfileMaxLayer(0),
      m_minClusterEnergy1(10.f), m_minDaughterEnergy1(1.f), m_minClusterEnergy2(10.f), m_minDaughterEnergy2(5.f),
      m_minClusterEnergy3(20.f), m_minDaughterEnergy3(10.f), m_maxNPeaks(5) {}
...
StatusCode PhotonSplittingAlgorithm::Run() {
  ...
  for (ClusterVector::const_iterator iter = clusterVector.begin(), iterEnd = clusterVector.end(); iter != iterEnd;
    ...
    if (pCluster->GetElectromagneticEnergy() > cutOffE && showersPhoton.size() < m_maxNPeaks) {
      int energyCounter(0);
      for (ShowerProfilePlugin::ShowerPeakList::const_iterator jIter = showersPhoton.begin(),
                                                               jIterEnd = showersPhoton.end();
    ...
    if (showersPhoton.size() > 1 && split) {
      ...
      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                               PandoraContentApi::InitializeFragmentation(
                                   *this, tempClusterList, originalClusterListName, tempClusterListName));
      ...
      try {
        for (int iPeak = 0, iPeakEnd = showersPhoton.size(); iPeak != iPeakEnd; ++iPeak) {
          ...
          PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                                  PandoraContentApi::Cluster::Create(*this, parameters, pNewCluster));
        ...
        usedCluster = false;
        std::cout << "PhotonSplittingAlgorithm::Run: StatusCodeException caught during fragmentation process."
                  << std::endl;
      }
...
StatusCode PhotonSplittingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxDistanceToTrackCut", m_maxDistanceToTrackCut));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinClusterEnergy1", m_minClusterEnergy1));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinDaughterEnergy1", m_minDaughterEnergy1));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinClusterEnergy2", m_minClusterEnergy2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinDaughterEnergy2", m_minDaughterEnergy2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinClusterEnergy3", m_minClusterEnergy3));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinDaughterEnergy3", m_minDaughterEnergy3));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxNPeaks", m_maxNPeaks));
