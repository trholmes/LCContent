...
PhotonRecoveryAlgorithm::PhotonRecoveryAlgorithm()
    : m_minElectromagneticEnergy(1.5f), m_maxInnerLayer(14), m_profileStartCut1(4.1f), m_profileStartEnergyCut(5.f),
      m_profileStartCut2(5.1f), m_profileStartCut3(2.75f), m_profileDiscrepancyCut1(0.4f),
      m_profileDiscrepancyEnergyCut(2.5f), m_profileDiscrepancyCutParameter1(0.5f),
      m_profileDiscrepancyCutParameter2(0.02f), m_minProfileDiscrepancy(0.f), m_profileDiscrepancyCut2(0.5f),
      m_maxOverlapInnerLayer(10), m_maxOverlapMipFraction(0.5f), m_minOverlapRadialDirectionCosine(0.9f),
      m_maxBarrelEndCapSplit(0.9f), m_softPhotonMinCaloHits(0), m_softPhotonMaxCaloHits(25), m_softPhotonMaxEnergy(1.f),
      m_softPhotonMaxInnerLayer(15), m_softPhotonMaxDCosR(0.9f), m_softPhotonLowEnergyCut(0.5f),
...
void PhotonRecoveryAlgorithm::FindPhotonsIdentifiedAsHadrons(const ClusterList* const pClusterList) const {
  for (ClusterList::const_iterator iter = pClusterList->begin(), iterEnd = pClusterList->end(); iter != iterEnd;
    ...
    if (electroMagneticEnergy > m_profileDiscrepancyEnergyCut) {
      profileDiscrepancyCut =
          m_profileDiscrepancyCutParameter1 - m_profileDiscrepancyCutParameter2 * showerProfileStart;
    }
    ...
    if ((showerProfileStart < profileStartCut) && (showerProfileDiscrepancy < profileDiscrepancyCut) &&
      ...
      isPhoton = true;
    } else if ((showerProfileDiscrepancy > m_minProfileDiscrepancy) &&
               (showerProfileDiscrepancy < m_profileDiscrepancyCut2) && (showerProfileStart < m_profileStartCut3)) {
      isPhoton = true;
    }
    ...
    if (!isPhoton) {
      ...
      if ((innerPseudoLayer < m_maxOverlapInnerLayer) &&
          ...
          (clusterFitResult.IsFitSuccessful()) &&
          (clusterFitResult.GetRadialDirectionCosine() > m_minOverlapRadialDirectionCosine) &&
...
StatusCode PhotonRecoveryAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      XmlHelper::ReadValue(xmlHandle, "MinElectromagneticEnergy", m_minElectromagneticEnergy));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxInnerLayer", m_maxInnerLayer));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ProfileStartCut1", m_profileStartCut1));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ProfileStartEnergyCut", m_profileStartEnergyCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ProfileStartCut2", m_profileStartCut2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ProfileStartCut3", m_profileStartCut3));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ProfileDiscrepancyCut1", m_profileDiscrepancyCut1));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ProfileDiscrepancyEnergyCut", m_profileDiscrepancyEnergyCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ProfileDiscrepancyCutParameter1", m_profileDiscrepancyCutParameter1));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ProfileDiscrepancyCutParameter2", m_profileDiscrepancyCutParameter2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinProfileDiscrepancy", m_minProfileDiscrepancy));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ProfileDiscrepancyCut2", m_profileDiscrepancyCut2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxOverlapInnerLayer", m_maxOverlapInnerLayer));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxOverlapMipFraction", m_maxOverlapMipFraction));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MinOverlapRadialDirectionCosine", m_minOverlapRadialDirectionCosine));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxBarrelEndCapSplit", m_maxBarrelEndCapSplit));

  // Soft photon id
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "SoftPhotonMinCaloHits", m_softPhotonMinCaloHits));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "SoftPhotonMaxCaloHits", m_softPhotonMaxCaloHits));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "SoftPhotonMaxEnergy", m_softPhotonMaxEnergy));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "SoftPhotonMaxInnerLayer", m_softPhotonMaxInnerLayer));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "SoftPhotonMaxDCosR", m_softPhotonMaxDCosR));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "SoftPhotonLowEnergyCut", m_softPhotonLowEnergyCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "SoftPhotonLowEnergyMaxDCosR", m_softPhotonLowEnergyMaxDCosR));
