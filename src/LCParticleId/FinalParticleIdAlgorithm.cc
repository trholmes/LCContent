...
StatusCode FinalParticleIdAlgorithm::Run() {
  ...
  for (PfoList::const_iterator iter = pPfoList->begin(), iterEnd = pPfoList->end(); iter != iterEnd; ++iter) {
    ...
    if (metadata.m_particleId.IsInitialized()) {
      ...
      metadata.m_energy =
          std::sqrt(metadata.m_mass.Get() * metadata.m_mass.Get() + pPfo->GetMomentum().GetMagnitudeSquared());
...
StatusCode FinalParticleIdAlgorithm::ReadSettings(const TiXmlHandle /*xmlHandle*/) { return STATUS_CODE_SUCCESS; }
