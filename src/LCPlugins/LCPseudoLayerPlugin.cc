...
LCPseudoLayerPlugin::LCPseudoLayerPlugin()
    : m_barrelInnerR(0.f), m_endCapInnerZ(0.f), m_barrelInnerRMuon(0.f), m_endCapInnerZMuon(0.f), m_rCorrection(0.f),
      m_zCorrection(0.f), m_rCorrectionMuon(0.f), m_zCorrectionMuon(0.f), m_barrelEdgeR(0.f), m_endCapEdgeZ(0.f) {}
...
StatusCode LCPseudoLayerPlugin::Initialize() {
  try {
    ...
    std::cout << "LCPseudoLayerPlugin: Incomplete geometry - consider using a different PseudoLayerCalculator."
              << std::endl;
...
unsigned int LCPseudoLayerPlugin::GetPseudoLayer(const CartesianVector& positionVector) const {
  ...
  const float rCoordinate(
      this->GetMaximumRadius(m_eCalBarrelAngleVector, positionVector.GetX(), positionVector.GetY()));
  const float rCoordinateMuon(
      this->GetMaximumRadius(m_muonBarrelAngleVector, positionVector.GetX(), positionVector.GetY()));
  ...
  if ((zCoordinate < m_endCapInnerZMuon) && (rCoordinateMuon < m_barrelInnerRMuon)) {
    const StatusCode statusCode(this->GetPseudoLayer(rCoordinate, zCoordinate, m_rCorrection, m_zCorrection,
                                                     m_barrelInnerR, m_endCapInnerZ, pseudoLayer));
...
StatusCode LCPseudoLayerPlugin::GetPseudoLayer(const float rCoordinate, const float zCoordinate,
                                               const float rCorrection, const float zCorrection,
                                               const float barrelInnerR, const float endCapInnerZ,
  ...
  if (zCoordinate < endCapInnerZ) {
    ...
    const StatusCode barrelStatusCode(
        this->FindMatchingLayer(rCoordinate - rCorrection, m_barrelLayerPositions, bestBarrelLayer));

    unsigned int bestEndCapLayer(0);
    const StatusCode endCapStatusCode(
        this->FindMatchingLayer(zCoordinate - zCorrection, m_endCapLayerPositions, bestEndCapLayer));
...
void LCPseudoLayerPlugin::StoreLayerPositions(const SubDetector& subDetector, LayerPositionList& layerPositionList) {
  ...
  for (SubDetector::SubDetectorLayerVector::const_iterator iter = subDetectorLayerVector.begin(),
...
void LCPseudoLayerPlugin::StoreDetectorOuterEdge() {
  ...
  m_barrelEdgeR = (std::max(pGeometryManager->GetSubDetector(ECAL_BARREL).GetOuterRCoordinate(),
                            std::max(pGeometryManager->GetSubDetector(HCAL_BARREL).GetOuterRCoordinate(),
                                     pGeometryManager->GetSubDetector(MUON_BARREL).GetOuterRCoordinate())));

  m_endCapEdgeZ = (std::max(std::fabs(pGeometryManager->GetSubDetector(ECAL_ENDCAP).GetOuterZCoordinate()),
                            std::max(std::fabs(pGeometryManager->GetSubDetector(HCAL_ENDCAP).GetOuterZCoordinate()),
                                     std::fabs(pGeometryManager->GetSubDetector(MUON_ENDCAP).GetOuterZCoordinate()))));

  if ((m_barrelLayerPositions.end() !=
       std::upper_bound(m_barrelLayerPositions.begin(), m_barrelLayerPositions.end(), m_barrelEdgeR)) ||
      (m_endCapLayerPositions.end() !=
       std::upper_bound(m_endCapLayerPositions.begin(), m_endCapLayerPositions.end(), m_endCapEdgeZ))) {
...
void LCPseudoLayerPlugin::StoreOverlapCorrectionDetails() {
  ...
  m_rCorrectionMuon =
      ((!IsEnclosingEndCap) ? 0.f : m_barrelInnerRMuon * ((m_endCapInnerZMuon / barrelOuterZMuon) - 1.f));
  m_zCorrectionMuon =
      ((IsEnclosingEndCap) ? 0.f : m_endCapInnerZMuon * ((m_barrelInnerRMuon / endCapOuterRMuon) - 1.f));
}
...
void LCPseudoLayerPlugin::FillAngleVector(const unsigned int symmetryOrder, const float phi0,
...
StatusCode LCPseudoLayerPlugin::ReadSettings(const TiXmlHandle /*xmlHandle*/) { return STATUS_CODE_SUCCESS; }
