...
LCBFieldPlugin::LCBFieldPlugin(const float innerBField, const float muonBarrelBField, const float muonEndCapBField)
    : m_innerBField(innerBField), m_muonBarrelBField(muonBarrelBField), m_muonEndCapBField(muonEndCapBField),
      m_muonEndCapInnerZ(std::numeric_limits<float>::max()), m_coilMidPointR(std::numeric_limits<float>::max()) {}
...
float LCBFieldPlugin::GetBField(const CartesianVector& positionVector) const {
  ...
  if (std::sqrt(positionVector.GetX() * positionVector.GetX() + positionVector.GetY() * positionVector.GetY()) >=
      m_coilMidPointR)
...
StatusCode LCBFieldPlugin::ReadSettings(const TiXmlHandle /*xmlHandle*/) { return STATUS_CODE_SUCCESS; }
