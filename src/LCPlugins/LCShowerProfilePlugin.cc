...
LCShowerProfilePlugin::LCShowerProfilePlugin()
    : m_showerStartMipFraction(0.8f), m_showerStartNonMipLayers(2), m_longProfileBinWidth(0.5f),
      m_longProfileNBins(100), m_longProfileMinCosAngle(0.3f), m_longProfileCriticalEnergy(0.08f),
      m_longProfileParameter0(1.25f), m_longProfileParameter1(0.5f), m_longProfileMaxDifference(0.1f),
      m_transProfileNBins(41), m_transProfilePeakThreshold(std::numeric_limits<float>::epsilon()),
      m_transProfilePeakFindingMetric(0), m_transProfileMinNBinsCut(2), m_transProfileTrackNearbyNSlices(3),
      m_transProfileMinTrackToPeakCut(1.6), m_transProfileMinDisTrackMatch(1.6) {}

//------------------------------------------------------------------------------------------------------------------------------------------

void LCShowerProfilePlugin::CalculateShowerStartLayer(const Cluster* const pCluster,
                                                      unsigned int& showerStartLayer) const {
...
void LCShowerProfilePlugin::CalculateLongitudinalProfile(const Cluster* const pCluster, float& profileStart,
                                                         float& profileDiscrepancy) const {
  ...
  for (unsigned int iLayer = innerPseudoLayer, outerPseudoLayer = pCluster->GetOuterPseudoLayer();
    ...
    for (CaloHitList::const_iterator hitIter = iter->second->begin(), hitIterEnd = iter->second->end();
      ...
      nRadiationLengthsInLayer +=
          (*hitIter)->GetNCellRadiationLengths() / std::max(cosOpeningAngle, m_longProfileMinCosAngle);
    }
  ...
  const unsigned int profileEndBin(
      std::min(static_cast<unsigned int>(nRadiationLengths / m_longProfileBinWidth), m_longProfileNBins));

  if ((0 == profileEndBin) || (eCalEnergy < std::numeric_limits<float>::epsilon()))
    throw StatusCodeException(STATUS_CODE_FAILURE);

  // 2. Construct expected cluster profile
  const double a(m_longProfileParameter0 +
                 m_longProfileParameter1 * std::log(clusterEnergy / m_longProfileCriticalEnergy));
...
void LCShowerProfilePlugin::CalculateTransverseProfile(const Cluster* const pCluster, const unsigned int maxPseudoLayer,
                                                       ShowerPeakList& showerPeakList, const bool inclusiveMode) const {
  ...
  this->ConvertBinsToShowerLists(showerProfile, showerPeakObjectVector, showerPeakList, showerPeakListNonPhoton,
                                 inclusiveMode);
...
void LCShowerProfilePlugin::CalculateTrackBasedTransverseProfile(const Cluster* const pCluster,
                                                                 const unsigned int maxPseudoLayer,
                                                                 const Track* const pMinTrack,
                                                                 const TrackVector& trackVector,
                                                                 ShowerPeakList& showerPeakListPhoton,
                                                                 ShowerPeakList& showerPeakListNonPhoton) const {
  ...
  this->CalculateTrackNearbyTransverseShowers(pCluster, pseudoLayerPerSlice, pMinTrack, trackVector, showerProfileFirst,
                                              showerPeakObjectVectorFirst, trackProjectionVector);
  ...
  if (m_transProfileTrackNearbyNSlices > 1) {
    for (unsigned int nIter = 1, nIterEnd = m_transProfileTrackNearbyNSlices; nIter != nIterEnd; ++nIter) {
      ...
      const unsigned int pseudoLayerEnd(
          nIter == m_transProfileTrackNearbyNSlices - 1 ? maxPseudoLayer : pseudoLayerPerSlice * (nIter + 1));

      TwoDShowerProfile showerProfileNext;
      ShowerPeakObjectVector showerPeakObjectVectorNext;
      this->CalculateTracklessTransverseShowers(pCluster, pseudoLayerEnd, showerProfileNext,
                                                showerPeakObjectVectorNext);
  ...
  this->ConvertBinsToShowerLists(showerProfileFirst, showerPeakObjectVectorFirst, showerPeakListPhoton,
                                 showerPeakListNonPhoton, false);
  std::sort(showerPeakListPhoton.begin(), showerPeakListPhoton.end(),
            LCShowerProfilePlugin::SortShowerPeakListByEnergy);
  std::sort(showerPeakListNonPhoton.begin(), showerPeakListNonPhoton.end(),
            LCShowerProfilePlugin::SortShowerPeakListByEnergy);
}

//------------------------------------------------------------------------------------------------------------------------------------------

void LCShowerProfilePlugin::CalculateTracklessTransverseShowers(const Cluster* const pCluster,
                                                                const unsigned int maxPseudoLayer,
                                                                TwoDShowerProfile& showerProfile,
...
void LCShowerProfilePlugin::CalculateTrackNearbyTransverseShowers(
    const Cluster* const pCluster, const unsigned int maxPseudoLayer, const Track* const pMinTrack,
    const TrackVector& trackVector, TwoDShowerProfile& showerProfile, ShowerPeakObjectVector& showerPeakObjectVector,
    TwoDBinVector& trackProjectionVector) const {
  this->InitialiseTransverseProfileWithTracks(pCluster, maxPseudoLayer, pMinTrack, trackVector, showerProfile,
                                              trackProjectionVector);
  this->ProcessShowerProfile(showerProfile, showerPeakObjectVector);
...
void LCShowerProfilePlugin::InitialiseTransverseProfile(const pandora::Cluster* const pCluster,
                                                        const unsigned int maxPseudoLayer,
                                                        TwoDShowerProfile& showerProfile) const {
...
void LCShowerProfilePlugin::InitialiseTransverseProfileWithTracks(
    const pandora::Cluster* const pCluster, const unsigned int maxPseudoLayer, const Track* const pMinTrack,
...
void LCShowerProfilePlugin::CalculateProjectionAxes(const Cluster* const pCluster, CartesianVector& innerLayerCentroid,
                                                    CartesianVector& uAxis, CartesianVector& vAxis,
                                                    const Track* const pMinTrack) const {
  innerLayerCentroid = pMinTrack ? (pMinTrack->GetTrackStateAtCalorimeter()).GetPosition()
  ...
  if (std::fabs(innerLayerCentroid.GetX()) < std::numeric_limits<float>::epsilon() &&
      std::fabs(innerLayerCentroid.GetY()) < std::numeric_limits<float>::epsilon()) {
...
void LCShowerProfilePlugin::InitialiseTwoDShowerProfile(const Cluster* const pCluster,
                                                        const unsigned int maxPseudoLayer,
                                                        const CartesianVector& innerLayerCentroid,
                                                        const CartesianVector& uAxis, const CartesianVector& vAxis,
                                                        TwoDShowerProfile& showerProfile) const {
  ...
  for (OrderedCaloHitList::const_iterator iter = orderedCaloHitList.begin(), iterEnd = orderedCaloHitList.end();
    ...
    for (CaloHitList::const_iterator hitIter = iter->second->begin(), hitIterEnd = iter->second->end();
      ...
      int uBin(0), vBin(0);
      this->FindHitPositionProjection(pCaloHit->GetPositionVector(), innerLayerCentroid, uAxis, vAxis, nOffsetBins,
                                      cellLengthScale, uBin, vBin);
...
void LCShowerProfilePlugin::FindTracksProjection(const Cluster* const pCluster, const TrackVector& trackVector,
                                                 const CartesianVector& innerLayerCentroid,
                                                 const CartesianVector& uAxis, const CartesianVector& vAxis,
                                                 TwoDBinVector& trackProjectionVector) const {
  ...
  for (TrackVector::const_iterator iter = trackVector.begin(), iterEnd = trackVector.end(); iter != iterEnd; ++iter) {
    ...
    if (!pTrack->CanFormClusterlessPfo() && !pTrack->CanFormPfo())
      continue;
    if (!(pTrack->GetDaughterList()).empty())
      continue;

    int uBin(0), vBin(0);
    this->FindHitPositionProjection((pTrack->GetTrackStateAtCalorimeter()).GetPosition(), innerLayerCentroid, uAxis,
                                    vAxis, nOffsetBins, cellLengthScale, uBin, vBin);
    if ((uBin >= 0) && (vBin >= 0) && (uBin < m_transProfileNBins) && (vBin < m_transProfileNBins)) {
...
void LCShowerProfilePlugin::FindRawPeaksInTwoDShowerProfile(TwoDShowerProfile& showerProfile,
                                                            ShowerPeakObjectVector& showerPeakObjectVector) const {
  for (int uBin = 0; uBin < m_transProfileNBins; ++uBin) {
    for (int vBin = 0; vBin < m_transProfileNBins; ++vBin) {
...
void LCShowerProfilePlugin::AssociateUnavailableBinsToPeaks(const TwoDShowerProfile& showerProfile,
                                                            ShowerPeakObjectVector& showerPeakObjectVector) const {
  for (int uBin = 0; uBin < m_transProfileNBins; ++uBin) {
    for (int vBin = 0; vBin < m_transProfileNBins; ++vBin) {
      if (showerProfile[uBin][vBin].m_isAvailable)
...
void LCShowerProfilePlugin::AssociateBinsToPeaks(const TwoDShowerProfile& showerProfile,
                                                 ShowerPeakObjectVector& showerPeakObjectVector) const {
  for (int uBin = 0; uBin < m_transProfileNBins; ++uBin) {
    for (int vBin = 0; vBin < m_transProfileNBins; ++vBin) {
      if (!showerProfile[uBin][vBin].m_isAvailable)
...
void LCShowerProfilePlugin::AssociateBinsToPeaks(const TwoDBinVector& twoDBinVector,
                                                 ShowerPeakObjectVector& showerPeakObjectVector) const {
  for (TwoDBinVector::const_iterator iter = twoDBinVector.begin(), iterEnd = twoDBinVector.end(); iter != iterEnd;
       ++iter) {
...
void LCShowerProfilePlugin::ApplyQualityCutPeakNBin(ShowerPeakObjectVector& showerPeakObjectVector,
                                                    TwoDBinVector& twoDBinVector) const {
  for (ShowerPeakObjectVector::iterator iter = showerPeakObjectVector.begin(), iterEnd = showerPeakObjectVector.end();
       iter != iterEnd; ++iter) {
    ShowerPeakObject& showerPeakObject(*iter);
    if (!this->PassQualityCutPeakNBin(showerPeakObject)) {
      twoDBinVector.insert(twoDBinVector.end(), showerPeakObject.m_associatedBins.begin(),
                           showerPeakObject.m_associatedBins.end());
...
void LCShowerProfilePlugin::MarkPeaksCloseToTracks(const TwoDBinVector& trackProjectionVector,
                                                   ShowerPeakObjectVector& showerPeakObjectVector) const {
  for (ShowerPeakObjectVector::iterator iter = showerPeakObjectVector.begin(), iterEnd = showerPeakObjectVector.end();
       iter != iterEnd; ++iter) {
    ShowerPeakObject& showerPeakObject(*iter);
    ...
    for (TwoDBinVector::const_iterator tIter = trackProjectionVector.begin(), tIterEnd = trackProjectionVector.end();
...
void LCShowerProfilePlugin::MatchPeaksInTwoSlices(const ShowerPeakObjectVector& showerPeakObjectVectorFirst,
                                                  ShowerPeakObjectVector& showerPeakObjectVectorNext) const {
  for (ShowerPeakObjectVector::iterator iter = showerPeakObjectVectorNext.begin(),
                                        iterEnd = showerPeakObjectVectorNext.end();
       iter != iterEnd; ++iter) {
    ...
    for (ShowerPeakObjectVector::const_iterator jIter = showerPeakObjectVectorFirst.begin(),
                                                jIterEnd = showerPeakObjectVectorFirst.end();
      ...
      const int distance2(uBinDistance * uBinDistance + vBinDistance * vBinDistance);
      if (distance2 < minDisTrackMatch2 &&
          (showerPeakObjectNext.GetPeakEnergy() > showerPeakObjectFirst.GetPeakEnergy() ||
           showerPeakObjectNext.GetPeakEnergy() == showerPeakObjectFirst.GetPeakEnergy())) {
...
void LCShowerProfilePlugin::ProcessShowerProfile(TwoDShowerProfile& showerProfile,
                                                 ShowerPeakObjectVector& showerPeakObjectVector) const {
...
void LCShowerProfilePlugin::FindHitPositionProjection(const CartesianVector& hitPosition,
                                                      const CartesianVector& innerLayerCentroid,
                                                      const CartesianVector& uAxis, const CartesianVector& vAxis,
                                                      const int nOffsetBins, const float cellLengthScale, int& uBin,
                                                      int& vBin) const {
  if (cellLengthScale < std::numeric_limits<float>::epsilon())
    throw StatusCodeException(STATUS_CODE_FAILURE);
...
void LCShowerProfilePlugin::FindBoundaryBins(const int uBin, const int vBin, const int uEdgeLow, const int uEdgeHigh,
                                             const int vEdgeLow, const int vEdgeHigh, int& uEdgeBin,
                                             int& vEdgeBin) const {
  ...
  if (theta >= -quarterPi && theta < quarterPi) {
    uEdgeBin = uEdgeHigh;
    vEdgeBin = std::max(std::min(vCentralBin + static_cast<int>((uEdgeHigh - uCentralBin) * std::tan(theta)), vEdgeLow),
                        vEdgeHigh);
  } else if (theta >= quarterPi && theta < 3 * quarterPi) {
    vEdgeBin = vEdgeHigh;
    uEdgeBin = std::max(std::min(uCentralBin + static_cast<int>((vEdgeHigh - vCentralBin) / std::tan(theta)), uEdgeLow),
                        uEdgeHigh);
  } else if (theta >= 3 * quarterPi && theta < 3 * -quarterPi) {
    uEdgeBin = uEdgeLow;
    vEdgeBin = std::max(std::min(vCentralBin + static_cast<int>((uCentralBin - uEdgeLow) * std::tan(theta)), vEdgeLow),
                        vEdgeHigh);
  } else if (theta >= 3 * -quarterPi && theta < -quarterPi) {
    vEdgeBin = vEdgeLow;
    uEdgeBin = std::max(std::min(uCentralBin + static_cast<int>((vCentralBin - vEdgeLow) / std::tan(theta)), uEdgeLow),
                        uEdgeHigh);
  }
}
...
void LCShowerProfilePlugin::ConvertBinsToShowerLists(const TwoDShowerProfile& showerProfile,
                                                     const ShowerPeakObjectVector& showerPeakObjectVector,
                                                     ShowerPeakList& showerPeakListPhoton,
                                                     ShowerPeakList& showerPeakListNonPhoton,
                                                     const bool inclusiveMode) const {
  for (ShowerPeakObjectVector::const_iterator iter = showerPeakObjectVector.begin(),
                                              iterEnd = showerPeakObjectVector.end();
       iter != iterEnd; ++iter) {
    ...
    for (TwoDBinVector::const_iterator bIter = showerPeakObject.m_associatedBins.begin(),
                                       bIterEnd = showerPeakObject.m_associatedBins.end();
         bIter != bIterEnd; ++bIter) {
      ...
      caloHitList.insert(caloHitList.end(), showerProfileEntry.m_caloHitList.begin(),
                         showerProfileEntry.m_caloHitList.end());

      if (inclusiveMode)
        caloHitList.insert(caloHitList.end(), showerProfileEntry.m_unusedCaloHitList.begin(),
                           showerProfileEntry.m_unusedCaloHitList.end());
    }
    ...
    const float peakRms((meanSquareDeviation > 0.f) ? std::sqrt(meanSquareDeviation)
                                                    : std::numeric_limits<float>::max());

    const float meanSquareDeviationU(uuBar - uBar * uBar);
    const float peakRmsU((meanSquareDeviationU > 0.f) ? std::sqrt(meanSquareDeviationU)
                                                      : std::numeric_limits<float>::max());
    const float meanSquareDeviationV(vvBar - vBar * vBar);
    const float peakRmsV((meanSquareDeviationV > 0.f) ? std::sqrt(meanSquareDeviationV)
                                                      : std::numeric_limits<float>::max());
...
bool LCShowerProfilePlugin::IsPeak(TwoDShowerProfile& showerProfile, const int uBin, const int vBin) const {
  return ((uBin <= 1 || uBin >= m_transProfileNBins - 2 || vBin <= 1 || vBin >= m_transProfileNBins - 2)
              ? this->Check8NeighbourFull(showerProfile, uBin, vBin)
              : this->Check8NeighbourFast(showerProfile, uBin, vBin));
}

//------------------------------------------------------------------------------------------------------------------------------------------

bool LCShowerProfilePlugin::Check8NeighbourFull(TwoDShowerProfile& showerProfile, const int uBin,
                                                const int vBin) const {
...
bool LCShowerProfilePlugin::Check8NeighbourFast(TwoDShowerProfile& showerProfile, const int uBin,
                                                const int vBin) const {
  for (int uBinStart = uBin - 1, uBinEnd = uBin + 1; uBinStart <= uBinEnd; ++uBinStart) {
    for (int vBinStart = vBin - 1, vBinEnd = vBin + 1; vBinStart <= vBinEnd; ++vBinStart) {
      if (uBinStart == uBin && vBinStart == vBin)
...
float LCShowerProfilePlugin::CalculatePeakFindingMetric(const float distance, const float energy) const {
  ...
  default:
    std::cout
        << "LCShowerProfilePlugin::CalculatePeakFindingMetric invalid parameter for m_transProfilePeakFindingMetric: "
        << m_transProfilePeakFindingMetric << std::endl;
    throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
  }
}
...
void LCShowerProfilePlugin::CalculateBestPeakUsingMetric(ShowerPeakObjectVector& showerPeakObjectVector, const int uBin,
                                                         const int vBin,
  ...
  for (ShowerPeakObjectVector::iterator iter = showerPeakObjectVector.begin(), iterEnd = showerPeakObjectVector.end();
    ...
    const int distance2((uBin - showerPeakObject->GetPeakUBin()) * (uBin - showerPeakObject->GetPeakUBin()) +
                        (vBin - showerPeakObject->GetPeakVBin()) * (vBin - showerPeakObject->GetPeakVBin()));
    const float metric(
        this->CalculatePeakFindingMetric(std::sqrt(static_cast<float>(distance2)), showerPeakObject->GetPeakEnergy()));
    if (metric < minMetric) {
      minMetric = metric;
...
StatusCode LCShowerProfilePlugin::ReadSettings(const TiXmlHandle xmlHandle) {
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ShowerStartMipFraction", m_showerStartMipFraction));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ShowerStartNonMipLayers", m_showerStartNonMipLayers));

  PANDORA_THROW_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                 XmlHelper::ReadValue(xmlHandle, "LongProfileBinWidth", m_longProfileBinWidth));

  if (m_longProfileBinWidth < std::numeric_limits<float>::epsilon())
    throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);

  PANDORA_THROW_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                 XmlHelper::ReadValue(xmlHandle, "LongProfileNBins", m_longProfileNBins));

  if (0 == m_longProfileNBins)
    throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);

  PANDORA_THROW_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                 XmlHelper::ReadValue(xmlHandle, "LongProfileMinCosAngle", m_longProfileMinCosAngle));

  PANDORA_THROW_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "LongProfileCriticalEnergy", m_longProfileCriticalEnergy));

  PANDORA_THROW_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                 XmlHelper::ReadValue(xmlHandle, "LongProfileParameter0", m_longProfileParameter0));

  PANDORA_THROW_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                 XmlHelper::ReadValue(xmlHandle, "LongProfileParameter1", m_longProfileParameter1));

  PANDORA_THROW_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "LongProfileMaxDifference", m_longProfileMaxDifference));

  PANDORA_THROW_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                 XmlHelper::ReadValue(xmlHandle, "TransProfileNBins", m_transProfileNBins));

  PANDORA_THROW_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "TransProfilePeakThreshold", m_transProfilePeakThreshold));

  PANDORA_THROW_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "TransProfilePeakFindingMetric", m_transProfilePeakFindingMetric));

  PANDORA_THROW_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                 XmlHelper::ReadValue(xmlHandle, "TransProfileMinNBinsCut", m_transProfileMinNBinsCut));

  PANDORA_THROW_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "TransProfileTrackNearbyNSlices", m_transProfileTrackNearbyNSlices));

  PANDORA_THROW_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "TransProfileMinTrackToPeakCut", m_transProfileMinTrackToPeakCut));

  PANDORA_THROW_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "TransProfileMinDisTrackMatch", m_transProfileMinDisTrackMatch));
