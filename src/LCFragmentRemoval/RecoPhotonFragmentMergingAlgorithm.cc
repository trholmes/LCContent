...
RecoPhotonFragmentMergingAlgorithm::RecoPhotonFragmentMergingAlgorithm()
    : m_energyRatioCandidatePeakToClusterNeutralThresholdLow1(0.7f), m_hitSeparationPhotonPhotonThresholdLow3(13.f),
...
StatusCode RecoPhotonFragmentMergingAlgorithm::GetAffectedClusterList(const ClusterList*& pClusterList) const {
  // Take non-photon clusters in current list, save into a separate list, process using topological associations, then
  // re-merge into original list
  ...
  for (ClusterList::const_iterator iter = pInputClusterList->begin(), iterEnd = pInputClusterList->end();
       iter != iterEnd; ++iter) {
  ...
  if (!nonPhotonClusterList.empty()) {
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                             PandoraContentApi::SaveList(*this, m_nonPhotonClusterListName, nonPhotonClusterList));
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                             PandoraContentApi::ReplaceCurrentList<Cluster>(*this, m_nonPhotonClusterListName));
  ...
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           PandoraContentApi::GetCurrentList(*this, pCurrentClusterList, currentClusterListName));

  if (!pCurrentClusterList->empty())
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                             PandoraContentApi::SaveList<Cluster>(*this, inputClusterListName));
...
bool RecoPhotonFragmentMergingAlgorithm::GetPhotonPhotonMergingFlag(const Parameters& parameters) const {
  if (parameters.m_energyOfCandidateCluster < m_lowEnergyOfCandidateClusterThreshold) {
    return (this->IsPhotonFragmentInShowerProfile(parameters) || this->IsAbsoluteLowEnergyPhotonFragment(parameters) ||
            this->IsSmallPhotonFragment1(parameters) || this->IsSmallPhotonFragment2(parameters));
  } else {
...
bool RecoPhotonFragmentMergingAlgorithm::GetPhotonNeutralMergingFlag(const Parameters& parameters) const {
  if (parameters.m_energyOfCandidateCluster < m_lowEnergyOfCandidateClusterThreshold) {
    return (this->IsNeutralFragmentInShowerProfile(parameters) || this->IsSmallNeutralFragment(parameters) ||
...
StatusCode RecoPhotonFragmentMergingAlgorithm::DeleteClusters(const ClusterVector& photonClusterVec,
...
StatusCode RecoPhotonFragmentMergingAlgorithm::DeleteClusters(const ClusterVector& clusterVec,
                                                              const bool nonFixedPhotonsOnly) const {
...
bool RecoPhotonFragmentMergingAlgorithm::IsAbsoluteLowEnergyPhotonFragment(const Parameters& parameters) const {
  return (!parameters.m_hasCrossedGap && parameters.m_weightedLayerSeparation > m_minWeightedLayerSeparation &&
...
bool RecoPhotonFragmentMergingAlgorithm::IsHighEnergyNeutralFragmentInShowerProfile(
...
bool RecoPhotonFragmentMergingAlgorithm::IsHighEnergyRelativeLowEnergyNeutralFragment(
    const Parameters& parameters) const {
  return (parameters.m_energyOfCandidateCluster > 0.f && parameters.m_energyOfMainCluster > 0.f &&
...
StatusCode RecoPhotonFragmentMergingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      XmlHelper::ProcessAlgorithmList(*this, xmlHandle, "associationAlgorithms", m_associationAlgorithms));

  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           XmlHelper::ReadValue(xmlHandle, "NonPhotonClusterListName", m_nonPhotonClusterListName));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle,
                                                       "EnergyRatioCandidatePeakToClusterNeutralThresholdLow1",
                                                       m_energyRatioCandidatePeakToClusterNeutralThresholdLow1));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ClosestDistanceSeparationPhotonPhotonThresholdLow3",
                                                       m_hitSeparationPhotonPhotonThresholdLow3));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ClosestDistanceSeparationPhotonPhotonThresholdLow4",
                                                       m_hitSeparationPhotonPhotonThresholdLow4));
