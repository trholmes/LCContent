...
BeamHaloMuonRemovalAlgorithm::BeamHaloMuonRemovalAlgorithm()
    : m_monitoring(false), m_displayRetainedClusters(false), m_displayRejectedClusters(false) {}
...
bool BeamHaloMuonRemovalAlgorithm::IsBeamHaloMuon(const pandora::Cluster* const pCluster) const {
  ...
  int maximumYBin(-1), maximumXBin(-1);
  ...
  for (int iX = 0; iX < nBinsX; ++iX) {
    for (int iY = 0; iY < nBinsY; ++iY) {
      ...
      if ((std::abs(iX - maximumXBin) < 2) && (std::abs(iY - maximumYBin) < 2) &&
          (iX != maximumXBin || iY != maximumYBin)) {
        if (contentXY >= 10)
          nSurround++;
  ...
  if (m_monitoring) {
    ...
    std::cout << " nFilledBins " << nFilledBins << " nLowBins " << nLowBins << " nHighBins " << nHighBins << " from "
              << nBinsX * nBinsY << std::endl;
    std::cout << " Isolation count " << nSurround << " hits = " << nSumSurround << std::endl;
  }

  if ((maximumValue > 10.f) && (nFilledBins <= 15) && (nLowBins <= 10) && (nHighBins <= 5) &&
      (nBinsX * nBinsY <= 600)) {
    isBeamHaloMuon = true;
  } else if ((maximumValue > 20.f) && (maximumValue < 100.f) && (nFilledBins <= 25) && (nLowBins <= 15) &&
             (nHighBins <= 10) && (nBinsX * nBinsY <= 250)) {
    isBeamHaloMuon = true;
  }
...
StatusCode BeamHaloMuonRemovalAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      XmlHelper::ReadValue(xmlHandle, "DisplayRetainedClusters", m_displayRetainedClusters));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "DisplayRejectedClusters", m_displayRejectedClusters));
