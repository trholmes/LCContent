...
MergeSplitPhotonsAlgorithm::MergeSplitPhotonsAlgorithm()
    : m_minShowerMaxCosAngle(0.98f), m_contactDistanceThreshold(2.f), m_minContactLayers(3), m_minContactFraction(0.5f),
      m_transProfileMaxLayer(30), m_acceptMaxSmallFragmentEnergy(0.2f), m_acceptMaxSubsidiaryPeakEnergy(0.5f),
      m_acceptFragmentEnergyRatio(0.05f), m_acceptSubsidiaryPeakEnergyRatio(0.1f), m_earlyTransProfileMaxLayer(20) {}
...
StatusCode MergeSplitPhotonsAlgorithm::Run() {
  ...
  for (ClusterVector::iterator iterI = clusterVector.begin(), iterIEnd = clusterVector.end(); iterI != iterIEnd;
       ++iterI) {
    ...
    for (ClusterVector::iterator iterJ = iterI + 1, iterJEnd = clusterVector.end(); iterJ != iterJEnd; ++iterJ) {
      ...
      const CartesianVector daughterShowerMaxCentroid(
          pDaughterCluster->GetCentroid(this->GetShowerMaxLayer(pDaughterCluster)));
      ...
      const StatusCode contactStatusCode(FragmentRemovalHelper::GetClusterContactDetails(
          pParentCluster, pDaughterCluster, m_contactDistanceThreshold, nContactLayers, contactFraction));

      if ((STATUS_CODE_SUCCESS == contactStatusCode) && (nContactLayers >= m_minContactLayers) &&
          (contactFraction > m_minContactFraction)) {
        ...
        PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                                 PandoraContentApi::InitializeFragmentation(*this, clusterList, originalClusterListName,
                                                                            mergedClusterListName));
        ...
        parameters.m_caloHitList.insert(parameters.m_caloHitList.end(),
                                        pParentCluster->GetIsolatedCaloHitList().begin(),
                                        pParentCluster->GetIsolatedCaloHitList().end());
        parameters.m_caloHitList.insert(parameters.m_caloHitList.end(),
                                        pDaughterCluster->GetIsolatedCaloHitList().begin(),
                                        pDaughterCluster->GetIsolatedCaloHitList().end());

        const Cluster* pMergedCluster = NULL;
        PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                                 PandoraContentApi::Cluster::Create(*this, parameters, pMergedCluster));
        ...
        const float smallFragmentEnergy(
            std::min(pDaughterCluster->GetElectromagneticEnergy(), pParentCluster->GetElectromagneticEnergy()));
        const float largeFragmentEnergy(
            std::max(pDaughterCluster->GetElectromagneticEnergy(), pParentCluster->GetElectromagneticEnergy()));
        ...
        if (acceptMerge && (smallFragmentEnergy > m_acceptMaxSmallFragmentEnergy)) {
          ...
          pShowerProfilePlugin->CalculateTransverseProfile(pMergedCluster, m_earlyTransProfileMaxLayer,
                                                           earlyShowerPeakList);

          const float earlySubsidiaryPeakEnergy(
              (earlyShowerPeakList.size() > 1) ? earlyShowerPeakList[1].GetPeakEnergy() : 0.f);
...
StatusCode MergeSplitPhotonsAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinShowerMaxCosAngle", m_minShowerMaxCosAngle));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "AcceptMaxSmallFragmentEnergy", m_acceptMaxSmallFragmentEnergy));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "AcceptMaxSubsidiaryPeakEnergy", m_acceptMaxSubsidiaryPeakEnergy));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "AcceptFragmentEnergyRatio", m_acceptFragmentEnergyRatio));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "AcceptSubsidiaryPeakEnergyRatio", m_acceptSubsidiaryPeakEnergyRatio));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "EarlyTransProfileMaxLayer", m_earlyTransProfileMaxLayer));
