...
ShowerMipMerging4Algorithm::ShowerMipMerging4Algorithm()
    : m_canMergeMinMipFraction(0.7f), m_canMergeMaxRms(5.f), m_minCaloHitsPerDaughterCluster(6),
      m_minCaloHitsPerParentCluster(11), m_maxLayerDifference(4), m_maxProjectionDistance(50.f),
      m_maxProjectionDistanceRatio(0.9f), m_nProjectionExaminationLayers(4), m_maxCentroidDistance(500.f) {}
...
StatusCode ShowerMipMerging4Algorithm::Run() {
  ...
  for (ClusterVector::iterator iterI = clusterVector.begin(), iterIEnd = clusterVector.end(); iterI != iterIEnd;
    ...
    for (ClusterVector::const_iterator iterJ = clusterVector.begin(), iterJEnd = clusterVector.end(); iterJ != iterJEnd;
      ...
      const unsigned int minDaughterInnerLayer(
          (parentOuterLayer > m_maxLayerDifference) ? parentOuterLayer - m_maxLayerDifference : 0);

      if (daughterInnerLayer < minDaughterInnerLayer)
        continue;

      // Get the smallest distance between a hit in the daughter cluster and the projected initial direction of the
      // parent cluster
    ...
    if (STATUS_CODE_SUCCESS !=
        ClusterHelper::GetDistanceToClosestCentroid(pBestParentCluster, pDaughterCluster, centroidDistance))
      continue;
...
float ShowerMipMerging4Algorithm::GetDistanceFromInitialProjection(const Cluster* const pClusterToProject,
                                                                   const Cluster* const pClusterToExamine) const {
...
StatusCode ShowerMipMerging4Algorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      XmlHelper::ReadValue(xmlHandle, "MinCaloHitsPerDaughterCluster", m_minCaloHitsPerDaughterCluster));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MinCaloHitsPerParentCluster", m_minCaloHitsPerParentCluster));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxLayerDifference", m_maxLayerDifference));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxProjectionDistance", m_maxProjectionDistance));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MaxProjectionDistanceRatio", m_maxProjectionDistanceRatio));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "NProjectionExaminationLayers", m_nProjectionExaminationLayers));
