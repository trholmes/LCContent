...
ShowerMipMerging2Algorithm::ShowerMipMerging2Algorithm()
    : m_canMergeMinMipFraction(0.7f), m_canMergeMaxRms(5.f), m_minHitsInCluster(4), m_minOccupiedLayersInCluster(2),
      m_fitToAllHitsChi2Cut(5.f), m_nPointsToFit(8), m_maxLayerDifference(6), m_maxCentroidDifference(2000.f),
      m_maxFitDirectionDotProduct(-0.8f), m_perpendicularDistanceCutFine(50.f), m_perpendicularDistanceCutCoarse(75.f) {
}
...
StatusCode ShowerMipMerging2Algorithm::Run() {
  ...
  for (ClusterVector::const_iterator iterI = clusterVector.begin(); iterI != clusterVector.end(); ++iterI) {
    ...
    if ((pParentCluster->GetNCaloHits() < m_minHitsInCluster) ||
        (pParentCluster->GetOrderedCaloHitList().size() < m_minOccupiedLayersInCluster))
      continue;
    ...
    for (ClusterVector::iterator iterJ = clusterVector.begin(); iterJ != clusterVector.end(); ++iterJ) {
      ...
      if (centroidDifference.GetUnitVector().GetDotProduct(parentClusterFitResult.GetDirection()) >
          m_maxFitDirectionDotProduct)
        continue;
      ...
      const float perpendicularDistanceCut((PandoraContentApi::GetGeometry(*this)->GetHitTypeGranularity(
                                                pDaughterCluster->GetInnerLayerHitType()) <= FINE)
                                               ? m_perpendicularDistanceCutFine
                                               : m_perpendicularDistanceCutCoarse);
      ...
      if ((perpendicularDistance < minPerpendicularDistance) || ((perpendicularDistance == minPerpendicularDistance) &&
                                                                 (daughterClusterEnergy < bestDaughterClusterEnergy))) {
    ...
    if (bestDaughterClusterIter != clusterVector.end()) {
      PANDORA_RETURN_RESULT_IF(
          STATUS_CODE_SUCCESS, !=,
          PandoraContentApi::MergeAndDeleteClusters(*this, pParentCluster, *bestDaughterClusterIter));
...
StatusCode ShowerMipMerging2Algorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "FitToAllHitsChi2Cut", m_fitToAllHitsChi2Cut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "NPointsToFit", m_nPointsToFit));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MaxFitDirectionDotProduct", m_maxFitDirectionDotProduct));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "PerpendicularDistanceCutFine", m_perpendicularDistanceCutFine));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "PerpendicularDistanceCutCoarse", m_perpendicularDistanceCutCoarse));
