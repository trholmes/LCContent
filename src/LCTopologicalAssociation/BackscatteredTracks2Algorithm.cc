...
BackscatteredTracks2Algorithm::BackscatteredTracks2Algorithm()
    : m_canMergeMinMipFraction(0.7f), m_canMergeMaxRms(5.f), m_minCaloHitsPerCluster(6), m_maxFitRms(15.f),
      m_nFitProjectionLayers(2), m_maxFitDistanceToClosestHit(30.f), m_maxCentroidDistance(1000.f) {}
...
StatusCode BackscatteredTracks2Algorithm::Run() {
  ...
  for (ClusterVector::const_iterator iterI = clusterVector.begin(), iterIEnd = clusterVector.end(); iterI != iterIEnd;
       ++iterI) {
    ...
    if (STATUS_CODE_SUCCESS !=
        ClusterFitHelper::FitLayers(pParentCluster, parentInnerLayer, parentShowerStartLayer, parentClusterFitResult))
      continue;
    ...
    for (ClusterVector::iterator iterJ = clusterVector.begin(), iterJEnd = clusterVector.end(); iterJ != iterJEnd;
         ++iterJ) {
      ...
      // Backscattered particle is expected to be daughter of a parent mip section; cut on overlap between relevant
      // layers
      ...
      // Cut on the distance of closest approach between the fit to the parent cluster and the daughter cluster
      // candidate
      const unsigned int daughterInnerLayer(pDaughterCluster->GetInnerPseudoLayer());
      const unsigned int fitProjectionOuterLayer(
      ...
      const float fitDistanceToClosestHit(ClusterHelper::GetDistanceToClosestHit(
          parentClusterFitResult, pDaughterCluster, daughterInnerLayer, fitProjectionOuterLayer));
