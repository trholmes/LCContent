...
HighEnergyPhotonRecoveryAlgorithm::HighEnergyPhotonRecoveryAlgorithm()
    : m_shouldUseCurrentClusterList(true), m_updateCurrentTrackClusterAssociations(true), m_numberContactLayers(2),
      m_centroidDistance2Cut(30000), m_fullClosestApproachCut(100.f), m_contactClosestApproachCut(100.f),
      m_minRmsRatioCut(0.3f), m_maxRmsRatioCut(5.f), m_daughterDistance2ToParentFitCut(2000.f), m_energyRatioCut(0.1f),
      m_fractionInConeCut(0.5f), m_coneCosineHalfAngle(0.9f), m_minCosConeAngleWrtRadial(0.25f),
...
StatusCode HighEnergyPhotonRecoveryAlgorithm::Run() {
  ...
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           this->PreSelectClusters(daughterVector, parentVector, parentCandidateMultiMap))
  ...
  for (ClusterVector::const_iterator iterI = daughterVector.begin(), iterIEnd = daughterVector.end(); iterI != iterIEnd;
       ++iterI) {
    ...
    if (STATUS_CODE_SUCCESS !=
        ClusterFitHelper::FitStart(pDaughterCluster, m_numberContactLayers, daughterFirstLayerFitResult))
      continue;
    ...
    for (auto iterJ = parentCandidateRange.first; iterJ != parentCandidateRange.second; ++iterJ)
      parentCandidates.push_back(iterJ->second);
    ...
    for (const Cluster* const pParentCluster : parentCandidates) {
      ...
      if (STATUS_CODE_SUCCESS !=
          ClusterFitHelper::FitEnd(pParentCluster, m_numberContactLayers, parentLastLayerFitResult))
        continue;
      ...
      if (rmsRatio > m_maxRmsRatioCut || rmsRatio < m_minRmsRatioCut)
        continue;

      float fullClosestApproach(std::numeric_limits<float>::max()),
          contactClosestApproach(std::numeric_limits<float>::max());
      if (STATUS_CODE_SUCCESS != ClusterHelper::GetFitResultsClosestApproach(
                                     daughterClusterFitResult, parentClusterFitResult, fullClosestApproach))
        continue;
      if (STATUS_CODE_SUCCESS != ClusterHelper::GetFitResultsClosestApproach(
                                     daughterFirstLayerFitResult, parentLastLayerFitResult, contactClosestApproach))
        continue;
      if (fullClosestApproach > m_fullClosestApproachCut && contactClosestApproach > m_contactClosestApproachCut)
        continue;

      const float daughterDistance2ToParentFit(
          this->GetHadEnergyWeightedDistance2ToLine(pDaughterCluster, parentClusterFitResult));
      if (daughterDistance2ToParentFit > m_daughterDistance2ToParentFitCut)
        continue;
...
StatusCode
HighEnergyPhotonRecoveryAlgorithm::GetClusterListAndNameMap(ClusterList& clusterList,
                                                            ClusterListToNameMap& clusterListToNameMap) const {
  if (m_shouldUseCurrentClusterList) {
    std::string clusterListName;
    const ClusterList* pClusterList = NULL;
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                             PandoraContentApi::GetCurrentList(*this, pClusterList, clusterListName));
...
StatusCode HighEnergyPhotonRecoveryAlgorithm::PrepareClusters(const ClusterList& clusterList,
                                                              ClusterVector& daughterVector,
                                                              ClusterVector& parentVector) const {
  for (ClusterList::const_iterator clusterIter = clusterList.begin(), clusterIterEnd = clusterList.end();
       clusterIter != clusterIterEnd; ++clusterIter) {
    const Cluster* pCluster = *clusterIter;
    if (PHOTON == pCluster->GetParticleId()) {
      if (ECAL == pCluster->GetInnerLayerHitType() && ECAL == pCluster->GetOuterLayerHitType() &&
          pCluster->GetElectromagneticEnergy() > 0.f) {
    ...
    if (pCluster->PassPhotonId(this->GetPandora())) {
      ...
      if (HCAL == pCluster->GetInnerLayerHitType() && HCAL == pCluster->GetOuterLayerHitType() &&
          pCluster->GetHadronicEnergy() > 0.f)
...
StatusCode HighEnergyPhotonRecoveryAlgorithm::PreSelectClusters(const ClusterVector& daughterVector,
                                                                const ClusterVector& parentVector,
                                                                ClusterClusterMultiMap& parentCandidateMultiMap) const {
  for (ClusterVector::const_iterator iterI = daughterVector.begin(), iterIEnd = daughterVector.end(); iterI != iterIEnd;
       ++iterI) {
    const Cluster* const pDaughterCluster = *iterI;
    ...
    for (ClusterVector::const_iterator iterJ = parentVector.begin(), iterJEnd = parentVector.end(); iterJ != iterJEnd;
         ++iterJ) {
      ...
      if (pDaughterCluster->GetHadronicEnergy() / pParentCluster->GetElectromagneticEnergy() > m_energyRatioCut)
        continue;
...
StatusCode HighEnergyPhotonRecoveryAlgorithm::MergeClusters(const ClusterClusterMap& daughterBestParentMap,
                                                            const ClusterListToNameMap& clusterListToNameMap) const {
  ClusterList clusterList;
  for (const auto& mapEntry : daughterBestParentMap)
  ...
  for (const Cluster* const pDaughterCluster : clusterList) {
    ...
    if (clusterListToNameMap.size() > 1) {
      ...
      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                               this->GetClusterListName(pBestParentCluster, clusterListToNameMap, parentListName));
      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                               this->GetClusterListName(pDaughterCluster, clusterListToNameMap, daughterListName));
      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
...
float HighEnergyPhotonRecoveryAlgorithm::GetFractionInCone(const Cluster* const pParentCluster,
...
StatusCode HighEnergyPhotonRecoveryAlgorithm::GetClusterListName(const Cluster* const pCluster,
                                                                 const ClusterListToNameMap& clusterListToNameMap,
                                                                 std::string& listName) const {
  for (ClusterListToNameMap::const_iterator iter = clusterListToNameMap.begin(), iterEnd = clusterListToNameMap.end();
       iter != iterEnd; ++iter) {
...
float HighEnergyPhotonRecoveryAlgorithm::GetHadEnergyWeightedDistance2ToLine(
    const Cluster* const pCluster, const ClusterFitResult& clusterFitResult) const {
  float energySum(0.f);
  ...
  for (OrderedCaloHitList::const_iterator iter = orderedCaloHitList.begin(), iterEnd = orderedCaloHitList.end();
    ...
    for (CaloHitList::const_iterator hIter = iter->second->begin(), hIterEnd = iter->second->end(); hIter != hIterEnd;
      ...
      weightedDistance2 +=
          this->GetDistance2ToLine(pCaloHit->GetPositionVector(), direction, intercept) * pCaloHit->GetHadronicEnergy();
    }
  }
...
float HighEnergyPhotonRecoveryAlgorithm::GetHadEnergyWeightedDistance2ToLine(const Cluster* const pCluster,
                                                                             const ClusterFitResult& clusterFitResult,
  ...
  for (CaloHitList::const_iterator hIter = iterLayer->second->begin(), hIterEnd = iterLayer->second->end();
...
float HighEnergyPhotonRecoveryAlgorithm::GetDistance2ToLine(const CartesianVector& point,
...
StatusCode HighEnergyPhotonRecoveryAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "UpdateCurrentTrackClusterAssociations",
                                                       m_updateCurrentTrackClusterAssociations));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "NumberContactLayers", m_numberContactLayers));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "CentroidDistance2Cut", m_centroidDistance2Cut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "FullClosestApproachCut", m_fullClosestApproachCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ContactClosestApproachCut", m_contactClosestApproachCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinRmsRatioCut", m_minRmsRatioCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxRmsRatioCut", m_maxRmsRatioCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "DaughterDistance2ToParentFitCut", m_daughterDistance2ToParentFitCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "EnergyRatioCut", m_energyRatioCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "FractionInConeCut", m_fractionInConeCut));
