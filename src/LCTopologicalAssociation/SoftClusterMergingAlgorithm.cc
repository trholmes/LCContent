...
SoftClusterMergingAlgorithm::SoftClusterMergingAlgorithm()
    : m_shouldUseCurrentClusterList(true), m_updateCurrentTrackClusterAssociations(true), m_maxHitsInSoftCluster(5),
      m_maxLayersSpannedBySoftCluster(3), m_maxHadEnergyForSoftClusterNoTrack(2.f), m_minClusterHadEnergy(0.25f),
      m_minClusterEMEnergy(0.025f), m_minCosOpeningAngle(0.f), m_minHitsInCluster(5), m_closestDistanceCut0(50.f),
      m_closestDistanceCut1(100.f), m_innerLayerCut1(20), m_closestDistanceCut2(250.f), m_innerLayerCut2(40),
      m_maxClusterDistanceFine(100.f), m_maxClusterDistanceCoarse(250.f), m_hitsToHitsCacheMap(new HitsToHitsCacheMap),
      m_hitNodes3D(new std::vector<HitKDNode3D>), m_hitsKdTree3D(new HitKDTree3D) {}
...
StatusCode SoftClusterMergingAlgorithm::Run() {
  ...
  for (const Cluster* const pDaughterCluster : clusterVector) {
    ...
    const int parentIndex(this->FindBestParentCluster(clusterVector, hitToClusterMap, quickUnion, pDaughterCluster,
                                                      theseHits, closestDistance));
...
void SoftClusterMergingAlgorithm::GetInputClusters(ClusterList& clusterList,
...
void SoftClusterMergingAlgorithm::GetInputCaloHits(const ClusterVector& clusterVector, CaloHitList& fullCaloHitList,
...
bool SoftClusterMergingAlgorithm::IsSoftCluster(const Cluster* const pDaughterCluster) const {
  ...
  if (nCaloHits <= m_maxHitsInSoftCluster) {
    isSoftCluster = true;
  } else if ((pDaughterCluster->GetOuterPseudoLayer() - pDaughterCluster->GetInnerPseudoLayer()) <
             m_maxLayersSpannedBySoftCluster) {
    isSoftCluster = true;
  ...
  if (pDaughterCluster->PassPhotonId(this->GetPandora()) &&
      (pDaughterCluster->GetElectromagneticEnergy() > m_minClusterEMEnergy))
    isSoftCluster = false;
...
int SoftClusterMergingAlgorithm::FindBestParentCluster(const ClusterVector& clusterVector,
                                                       const HitToClusterMap& hitToClusterMap, QuickUnion& quickUnion,
                                                       const Cluster* const pDaughterCluster,
                                                       const CaloHitList& daughterHits, float& closestDistance) const {
  int bestParentIndex(-1);
  closestDistance = std::numeric_limits<float>::max();

  const float searchDistance(
      (PandoraContentApi::GetGeometry(*this)->GetHitTypeGranularity(pDaughterCluster->GetOuterLayerHitType()) <= FINE)
          ? m_maxClusterDistanceFine
          : m_maxClusterDistanceCoarse);
  ...
  for (const CaloHit* const pCaloHitI : daughterHits) {
    ...
    if (m_hitsToHitsCacheMap->count(pCaloHitI)) {
      ...
      KDTreeCube searchRegionHits =
          build_3d_kd_search_region(pCaloHitI, searchDistance, searchDistance, searchDistance);
    ...
    for (const CaloHit* const pCaloHitJ : nearby_hits) {
      ...
      if ((distanceSquared < minDistanceSquared) ||
          ((distanceSquared == minDistanceSquared) && (clusterEnergyJ > bestParentClusterEnergy))) {
...
bool SoftClusterMergingAlgorithm::CanMergeSoftCluster(const Cluster* const pDaughterCluster,
                                                      const float closestDistance) const {
  ...
  return ((pDaughterCluster->GetHadronicEnergy() < m_minClusterHadEnergy) ||
          (pDaughterCluster->GetNCaloHits() < m_minHitsInCluster));
}

//------------------------------------------------------------------------------------------------------------------------------------------

void SoftClusterMergingAlgorithm::MergeClusters(const Cluster* const pParentCluster,
                                                const Cluster* const pDaughterCluster,
  ...
  if (clusterListToNameMap.size() > 1) {
    ...
    PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                            this->GetClusterListName(pParentCluster, clusterListToNameMap, parentListName));
    PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                            this->GetClusterListName(pDaughterCluster, clusterListToNameMap, daughterListName));
    PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                            PandoraContentApi::MergeAndDeleteClusters(*this, pParentCluster, pDaughterCluster,
...
StatusCode SoftClusterMergingAlgorithm::GetClusterListName(const Cluster* const pCluster,
...
StatusCode SoftClusterMergingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxHitsInSoftCluster", m_maxHitsInSoftCluster));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MaxLayersSpannedBySoftCluster", m_maxLayersSpannedBySoftCluster));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MaxHadEnergyForSoftClusterNoTrack", m_maxHadEnergyForSoftClusterNoTrack));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinClusterHadEnergy", m_minClusterHadEnergy));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinClusterEMEnergy", m_minClusterEMEnergy));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ClosestDistanceCut0", m_closestDistanceCut0));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ClosestDistanceCut1", m_closestDistanceCut1));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "InnerLayerCut1", m_innerLayerCut1));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ClosestDistanceCut2", m_closestDistanceCut2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "InnerLayerCut2", m_innerLayerCut2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxClusterDistanceFine", m_maxClusterDistanceFine));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MaxClusterDistanceCoarse", m_maxClusterDistanceCoarse));
