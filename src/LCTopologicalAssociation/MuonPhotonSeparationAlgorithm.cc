...
MuonPhotonSeparationAlgorithm::MuonPhotonSeparationAlgorithm() : m_highEnergyMuonCut(0.f), m_nTransitionLayers(1) {
...
StatusCode MuonPhotonSeparationAlgorithm::PerformFragmentation(const Cluster* const pOriginalCluster,
  ...
  if ((NULL != pMipCluster) && (NULL != pPhotonCluster) &&
    ...
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                             PandoraContentApi::RemoveTrackClusterAssociation(*this, pTrack, pOriginalCluster));
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                             PandoraContentApi::AddTrackClusterAssociation(*this, pTrack, pMipCluster));

    if (PandoraContentApi::GetPlugins(*this)->GetParticleId()->IsMuon(pMipCluster) &&
        (pTrack->GetEnergyAtDca() > m_highEnergyMuonCut)) {
      clusterListToSaveName = fragmentClustersListName;
      clusterListToDeleteName = originalClustersListName;
    } else {
      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                               PandoraContentApi::RemoveTrackClusterAssociation(*this, pTrack, pMipCluster));
      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                               PandoraContentApi::AddTrackClusterAssociation(*this, pTrack, pOriginalCluster));
    }
  }
...
StatusCode MuonPhotonSeparationAlgorithm::MakeClusterFragments(const unsigned int showerStartLayer,
  ...
  for (OrderedCaloHitList::const_iterator iter = orderedCaloHitList.begin(), iterEnd = orderedCaloHitList.end();
    ...
    for (CaloHitList::const_iterator hitIter = iter->second->begin(), hitIterEnd = iter->second->end();
      ...
      if (isHitOnMipPath || ((iLayer + m_nTransitionLayers) < showerStartLayer) ||
          (iLayer > (showerEndLayer + m_nTransitionLayers))) {
        if (NULL == pMipCluster) {
...
StatusCode MuonPhotonSeparationAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "HighEnergyMuonCut", m_highEnergyMuonCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "NTransitionLayers", m_nTransitionLayers));
