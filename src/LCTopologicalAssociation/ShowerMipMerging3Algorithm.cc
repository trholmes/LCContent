...
ShowerMipMerging3Algorithm::ShowerMipMerging3Algorithm()
    : m_canMergeMinMipFraction(0.7f), m_canMergeMaxRms(5.f), m_minCaloHitsInDaughter(6),
      m_minOccupiedLayersInDaughter(4), m_minCaloHitsInParent(3), m_nPointsToFit(10), m_maxFitChi2(10.f),
      m_nFitProjectionLayers(4), m_maxFitDistanceToClosestHit(30.f), m_maxClusterApproach(250.f) {}
  ...
  for (ClusterVector::iterator iterI = clusterVector.begin(), iterIEnd = clusterVector.end(); iterI != iterIEnd;
    ...
    for (ClusterVector::const_iterator iterJ = clusterVector.begin(), iterJEnd = clusterVector.end(); iterJ != iterJEnd;
      ...
      const unsigned int fitProjectionInnerLayer(
          (parentOuterLayer > m_nFitProjectionLayers) ? parentOuterLayer - m_nFitProjectionLayers : 0);
      const float fitDistanceToClosestHit(ClusterHelper::GetDistanceToClosestHit(
          daughterClusterFitResult, pParentCluster, fitProjectionInnerLayer, parentOuterLayer));
...
StatusCode ShowerMipMerging3Algorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinCaloHitsInDaughter", m_minCaloHitsInDaughter));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MinOccupiedLayersInDaughter", m_minOccupiedLayersInDaughter));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinCaloHitsInParent", m_minCaloHitsInParent));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "NPointsToFit", m_nPointsToFit));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxFitChi2", m_maxFitChi2));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxClusterApproach", m_maxClusterApproach));
