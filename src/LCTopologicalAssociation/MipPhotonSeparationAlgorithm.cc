...
MipPhotonSeparationAlgorithm::MipPhotonSeparationAlgorithm()
    : m_nLayersForMipRegion(2), m_nLayersForShowerRegion(2), m_maxLayersMissed(1), m_minMipRegion2Span(4),
      m_maxShowerStartLayer(20), m_minShowerRegionSpan(4), m_maxShowerStartLayer2(5), m_minShowerRegionSpan2(200),
      m_nonPhotonDeltaChi2Cut(0.f), m_photonDeltaChi2Cut(1.f), m_minHitsInPhotonCluster(6), m_genericDistanceCut(1.f),
      m_trackPathWidth(2.f), m_maxTrackSeparation2(1000.f * 1000.f), m_additionalPadWidthsFine(2.5f),
  ...
  for (ClusterVector::iterator iter = clusterVector.begin(), iterEnd = clusterVector.end(); iter != iterEnd; ++iter) {
    ...
    *iter = NULL;
    ...
    unsigned int showerStartLayer(std::numeric_limits<unsigned int>::max()),
        showerEndLayer(std::numeric_limits<unsigned int>::max());

    if (!this->ShouldFragmentCluster(pCluster, pTrack, showerStartLayer, showerEndLayer))
      continue;

    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                             this->PerformFragmentation(pCluster, pTrack, showerStartLayer, showerEndLayer));
  }
...
bool MipPhotonSeparationAlgorithm::ShouldFragmentCluster(const Cluster* const pCluster, const Track* const pTrack,
                                                         unsigned int& showerStartLayer,
                                                         unsigned int& showerEndLayer) const {
  const unsigned int firstPseudoLayer(
      PandoraContentApi::GetPlugins(*this)->GetPseudoLayerPlugin()->GetPseudoLayerAtIp());
  ...
  for (unsigned int iLayer = firstPseudoLayer; (iLayer <= lastPseudoLayer) && shouldContinue; ++iLayer) {
    ...
    if (orderedCaloHitList.end() != hitListIter) {
      for (CaloHitList::const_iterator iter = hitListIter->second->begin(), iterEnd = hitListIter->second->end();
           iter != iterEnd; ++iter) {
  ...
  if (((mipRegion2EndLayer != maxPseudoLayer) && (mipRegion2EndLayer - mipRegion2StartLayer > m_minMipRegion2Span)) &&
      ((showerStartLayer < m_maxShowerStartLayer) &&
       (((showerEndLayer != maxPseudoLayer) && (showerEndLayer - showerStartLayer > m_minShowerRegionSpan))))) {
    return true;
  }

  if (((showerStartLayer < m_maxShowerStartLayer2) &&
       (((showerEndLayer != maxPseudoLayer) && (showerEndLayer - showerStartLayer > m_minShowerRegionSpan2))))) {
    return true;
  }
...
StatusCode MipPhotonSeparationAlgorithm::PerformFragmentation(const Cluster* const pOriginalCluster,
                                                              const Track* const pTrack, unsigned int showerStartLayer,
                                                              unsigned int showerEndLayer) const {
  const ClusterList clusterList(1, pOriginalCluster);
  std::string originalClustersListName, fragmentClustersListName;

  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           PandoraContentApi::InitializeFragmentation(*this, clusterList, originalClustersListName,
  ...
  PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=,
      this->MakeClusterFragments(showerStartLayer, showerEndLayer, pOriginalCluster, pMipCluster, pPhotonCluster));
  ...
  if ((NULL != pMipCluster) && (NULL != pPhotonCluster) &&
      (pPhotonCluster->GetNCaloHits() >= m_minHitsInPhotonCluster)) {
    const float trackEnergy(pTrack->GetEnergyAtDca());
    const float originalChi(ReclusterHelper::GetTrackClusterCompatibility(
        this->GetPandora(), pOriginalCluster->GetTrackComparisonEnergy(this->GetPandora()), trackEnergy));
    const float newChi(ReclusterHelper::GetTrackClusterCompatibility(
        this->GetPandora(), pMipCluster->GetTrackComparisonEnergy(this->GetPandora()), trackEnergy));
    const float dChi2(newChi * newChi - originalChi * originalChi);

    const bool passChi2Cuts((dChi2 < m_nonPhotonDeltaChi2Cut) ||
                            (pPhotonCluster->PassPhotonId(this->GetPandora()) && (dChi2 < m_photonDeltaChi2Cut)));
...
StatusCode MipPhotonSeparationAlgorithm::MakeClusterFragments(const unsigned int showerStartLayer,
                                                              const unsigned int showerEndLayer,
                                                              const Cluster* const pOriginalCluster,
                                                              const Cluster*& pMipCluster,
                                                              const Cluster*& pPhotonCluster) const {
  ...
  for (OrderedCaloHitList::const_iterator iter = orderedCaloHitList.begin(), iterEnd = orderedCaloHitList.end();
    ...
    for (CaloHitList::const_iterator hitIter = iter->second->begin(), hitIterEnd = iter->second->end();
      ...
      PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_UNCHANGED, !=,
                                      this->GetDistanceToTrack(pOriginalCluster, pTrack, pCaloHit, distance));
      ...
      if ((distance < m_genericDistanceCut) || (iLayer < showerStartLayer) || (iLayer > showerEndLayer)) {
        if (NULL == pMipCluster) {
          ...
          PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                                   PandoraContentApi::Cluster::Create(*this, parameters, pMipCluster));
        }

        PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                                 PandoraContentApi::AddToCluster(*this, pMipCluster, pCaloHit));
      } else {
        if (NULL == pPhotonCluster) {
          PandoraContentApi::Cluster::Parameters parameters;
          parameters.m_caloHitList.push_back(pCaloHit);
          PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                                   PandoraContentApi::Cluster::Create(*this, parameters, pPhotonCluster));
        } else {
          PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                                   PandoraContentApi::AddToCluster(*this, pPhotonCluster, pCaloHit));
        }
      }
    }
  }
...
StatusCode MipPhotonSeparationAlgorithm::GetDistanceToTrack(const Cluster* const pCluster, const Track* const pTrack,
                                                            const CaloHit* const pCaloHit, float& distance) const {
...
StatusCode MipPhotonSeparationAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "NLayersForMipRegion", m_nLayersForMipRegion));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "NLayersForShowerRegion", m_nLayersForShowerRegion));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxLayersMissed", m_maxLayersMissed));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinMipRegion2Span", m_minMipRegion2Span));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxShowerStartLayer", m_maxShowerStartLayer));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinShowerRegionSpan", m_minShowerRegionSpan));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxShowerStartLayer2", m_maxShowerStartLayer2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinShowerRegionSpan2", m_minShowerRegionSpan2));

  // Parameters aiding selection of original clusters or new fragments
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "NonPhotonDeltaChi2Cut", m_nonPhotonDeltaChi2Cut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "PhotonDeltaChi2Cut", m_photonDeltaChi2Cut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinHitsInPhotonCluster", m_minHitsInPhotonCluster));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxTrackSeparation", maxTrackSeparation));
