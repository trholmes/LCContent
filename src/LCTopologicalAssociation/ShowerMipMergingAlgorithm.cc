...
ShowerMipMergingAlgorithm::ShowerMipMergingAlgorithm()
    : m_canMergeMinMipFraction(0.7f), m_canMergeMaxRms(5.f), m_minHitsInCluster(6), m_minOccupiedLayersInCluster(2),
      m_nPointsToFit(10), m_mipFractionCut(0.5f), m_fitToAllHitsRmsCut(10.f), m_maxCentroidDifference(1000.f),
      m_nFitProjectionLayers(5), m_maxDistanceToClosestHit(100.f), m_maxDistanceToClosestCentroid(25.f) {}
...
StatusCode ShowerMipMergingAlgorithm::Run() {
  ...
  for (ClusterVector::const_iterator iterI = clusterVector.begin(); iterI != clusterVector.end(); ++iterI) {
    ...
    if (!pParentCluster->GetFitToAllHitsResult().IsFitSuccessful() ||
        (pParentCluster->GetFitToAllHitsResult().GetRms() > m_fitToAllHitsRmsCut))
      continue;
    ...
    for (ClusterVector::iterator iterJ = clusterVector.begin(); iterJ != clusterVector.end(); ++iterJ) {
      ...
      const float distanceToClosestHit(ClusterHelper::GetDistanceToClosestHit(
          parentClusterFitResult, pDaughterCluster, parentOuterLayer, parentOuterLayer + m_nFitProjectionLayers));
      ...
      const float distanceToClosestCentroid(ClusterHelper::GetDistanceToClosestCentroid(
          parentClusterFitResult, pDaughterCluster, parentOuterLayer, parentOuterLayer + m_nFitProjectionLayers));

      const float daughterClusterEnergy(pDaughterCluster->GetHadronicEnergy());

      if ((distanceToClosestCentroid < minDistanceToCentroid) ||
          ((distanceToClosestCentroid == minDistanceToCentroid) &&
...
StatusCode ShowerMipMergingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MipFractionCut", m_mipFractionCut));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MaxDistanceToClosestHit", m_maxDistanceToClosestHit));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MaxDistanceToClosestCentroid", m_maxDistanceToClosestCentroid));
