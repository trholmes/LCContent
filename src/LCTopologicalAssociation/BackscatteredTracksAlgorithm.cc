...
BackscatteredTracksAlgorithm::BackscatteredTracksAlgorithm()
    : m_canMergeMinMipFraction(0.7f), m_canMergeMaxRms(5.f), m_minCaloHitsPerCluster(6), m_fitToAllHitsRmsCut(10.f),
      m_nOuterFitExclusionLayers(2), m_nFitProjectionLayers(2), m_maxFitDistanceToClosestHit(30.f),
...
StatusCode BackscatteredTracksAlgorithm::Run() {
  ...
  for (ClusterVector::iterator iterI = clusterVector.begin(), iterIEnd = clusterVector.end(); iterI != iterIEnd;
    ...
    if (!pDaughterCluster->GetFitToAllHitsResult().IsFitSuccessful() ||
        (pDaughterCluster->GetFitToAllHitsResult().GetRms() > m_fitToAllHitsRmsCut))
      continue;
    ...
    const unsigned int daughterOuterFitLayer(
        (daughterOuterLayer > m_nOuterFitExclusionLayers) ? daughterOuterLayer - m_nOuterFitExclusionLayers : 0);

    ClusterFitResult daughterClusterFitResult;
    if (STATUS_CODE_SUCCESS != ClusterFitHelper::FitLayers(pDaughterCluster, daughterInnerLayer, daughterOuterFitLayer,
                                                           daughterClusterFitResult))
      continue;
    ...
    for (ClusterVector::const_iterator iterJ = clusterVector.begin(), iterJEnd = clusterVector.end(); iterJ != iterJEnd;
      ...
      if ((pParentCluster->GetOuterPseudoLayer() <= daughterOuterLayer) ||
          (pParentCluster->GetInnerPseudoLayer() >= daughterOuterLayer))
        continue;
      ...
      if (STATUS_CODE_SUCCESS !=
          ClusterHelper::GetDistanceToClosestCentroid(pParentCluster, pDaughterCluster, centroidDistance))
        continue;
      ...
      const unsigned int fitProjectionInnerLayer(
          (daughterOuterLayer > m_nFitProjectionLayers) ? daughterOuterLayer - m_nFitProjectionLayers : 0);
      const unsigned int fitProjectionOuterLayer(daughterOuterLayer + m_nFitProjectionLayers);

      const float fitDistanceToClosestHit(ClusterHelper::GetDistanceToClosestHit(
          daughterClusterFitResult, pParentCluster, fitProjectionInnerLayer, fitProjectionOuterLayer));

      const float parentClusterEnergy(pParentCluster->GetHadronicEnergy());

      if ((fitDistanceToClosestHit < minFitDistanceToClosestHit) ||
          ((fitDistanceToClosestHit == minFitDistanceToClosestHit) &&
           (parentClusterEnergy > bestParentClusterEnergy))) {
...
StatusCode BackscatteredTracksAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "FitToAllHitsRmsCut", m_fitToAllHitsRmsCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "NOuterFitExclusionLayers", m_nOuterFitExclusionLayers));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "NFitProjectionLayers", m_nFitProjectionLayers));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MaxFitDistanceToClosestHit", m_maxFitDistanceToClosestHit));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxCentroidDistance", m_maxCentroidDistance));
