...
BrokenTracksAlgorithm::BrokenTracksAlgorithm()
    : m_canMergeMinMipFraction(0.7f), m_minHitsInCluster(4), m_minOccupiedLayersForStartFit(4),
      m_minOccupiedLayersForEndFit(2), m_nStartLayersToFit(5), m_nEndLayersToFit(8), m_maxFitRms(35.f),
      m_fitDirectionDotProductCut(0.5f), m_trackMergeCutFine(45.f), m_trackMergeCutCoarse(45.f),
      m_trackMergePerpCutFine(50.f), m_trackMergePerpCutCoarse(75.f), m_maxLayerDifference(10),
      m_maxCentroidDifference(2000.f), m_shouldPerformGapCheck(true), m_maxChi2ForGapCheck(2.f) {}
...
StatusCode BrokenTracksAlgorithm::Run() {
  ...
  for (ClusterFitRelationList::const_iterator iterI = clusterFitRelationList.begin(),
    ...
    for (ClusterFitRelationList::const_iterator iterJ = clusterFitRelationList.begin(),
                                                iterJEnd = clusterFitRelationList.end();
      ...
      const bool isDaughterFineGranularity(PandoraContentApi::GetGeometry(*this)->GetHitTypeGranularity(
                                               pDaughterCluster->GetInnerLayerHitType()) <= FINE);
      ...
      const CartesianVector parentCrossProduct(
          parentClusterFitResult.GetDirection().GetCrossProduct(centroidDifference));
      const float parentPerpendicularDistance(parentCrossProduct.GetMagnitude());

      const CartesianVector daughterCrossProduct(
          daughterClusterFitResult.GetDirection().GetCrossProduct(centroidDifference));
      ...
      if ((daughterInnerLayer - parentOuterLayer > m_maxLayerDifference) ||
          (centroidSeparation > m_maxCentroidDifference)) {
        if (!m_shouldPerformGapCheck)
          continue;

        if (parentClusterFitResult.GetChi2() > m_maxChi2ForGapCheck ||
            daughterClusterFitResult.GetChi2() > m_maxChi2ForGapCheck)
          continue;

        if (!ClusterHelper::DoesFitCrossGapRegion(this->GetPandora(), parentClusterFitResult, parentOuterCentroid,
                                                  centroidSeparation) &&
            !ClusterHelper::DoesFitCrossGapRegion(this->GetPandora(), daughterClusterFitResult, daughterInnerCentroid,
                                                  -centroidSeparation)) {
          continue;
        }
      }
...
StatusCode BrokenTracksAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      XmlHelper::ReadValue(xmlHandle, "MinOccupiedLayersForStartFit", m_minOccupiedLayersForStartFit));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MinOccupiedLayersForEndFit", m_minOccupiedLayersForEndFit));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "NStartLayersToFit", m_nStartLayersToFit));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "NEndLayersToFit", m_nEndLayersToFit));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxFitRms", m_maxFitRms));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "FitDirectionDotProductCut", m_fitDirectionDotProductCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "TrackMergeCutFine", m_trackMergeCutFine));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "TrackMergeCutCoarse", m_trackMergeCutCoarse));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "TrackMergePerpCutFine", m_trackMergePerpCutFine));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "TrackMergePerpCutCoarse", m_trackMergePerpCutCoarse));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxLayerDifference", m_maxLayerDifference));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxChi2ForGapCheck", m_maxChi2ForGapCheck));
