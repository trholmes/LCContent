...
MuonClusterAssociationAlgorithm::MuonClusterAssociationAlgorithm()
    : m_dCosCut(0.95f), m_minHitsInMuonCluster(3), m_shouldEstimateEnergyLostInCoil(true),
      m_coilCorrectionMinInnerRadius(4000.f), m_coilCorrectionMinInnerLayerHits(3), m_coilEnergyLossCorrection(10.f),
      m_minClusterHadronicEnergy(0.25f), m_minHitsInCluster(5), m_clusterAssociationChi(3.f) {}
...
StatusCode MuonClusterAssociationAlgorithm::Run() {
  ...
  // Will create target cluster list if target cluster list is not initialised, and there is a candidateMuonClusters to
  // save
  ...
  if (STATUS_CODE_NOT_INITIALIZED == targetListStatusCode || NULL == pTargetClusterList) {
    if (!candidateMuonClusters.empty()) {
      PANDORA_RETURN_RESULT_IF(
          STATUS_CODE_SUCCESS, !=,
          PandoraContentApi::SaveList(*this, m_muonClusterListName, m_targetClusterListName, candidateMuonClusters));
    }
    return STATUS_CODE_SUCCESS;
  ...
  for (ClusterVector::iterator iterI = muonClusterVector.begin(), iterIEnd = muonClusterVector.end(); iterI != iterIEnd;
       ++iterI) {
    ...
    if (m_shouldEstimateEnergyLostInCoil) {
      ...
      const unsigned int nHitsInInnerLayer(
          pMuonCluster->GetOrderedCaloHitList().GetNCaloHitsInPseudoLayer(muonClusterInnerLayer));
    ...
    for (ClusterList::const_iterator iterJ = pTargetClusterList->begin(), iterJEnd = pTargetClusterList->end();
      ...
      if (isLeavingDetector && hasAssociatedTrack) {
        if ((dCos > bestDCosLeavingTrack) ||
            ((dCos == bestDCosLeavingTrack) && (clusterEnergy > bestEnergyLeavingTrack))) {
          const float chi(ReclusterHelper::GetTrackClusterCompatibility(
              this->GetPandora(), pCluster->GetTrackComparisonEnergy(this->GetPandora()) + energyLostInCoil,
              trackEnergySum));
      ...
      if (!isLeavingDetector && hasAssociatedTrack) {
        if ((dCos > bestDCosNonLeavingTrack) ||
            ((dCos == bestDCosNonLeavingTrack) && (clusterEnergy > bestEnergyNonLeavingTrack))) {
          const float oldChi(ReclusterHelper::GetTrackClusterCompatibility(
              this->GetPandora(), pCluster->GetTrackComparisonEnergy(this->GetPandora()), trackEnergySum));
    ...
    if (NULL != pBestTargetCluster) {
      *iterI = NULL;
      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                               PandoraContentApi::MergeAndDeleteClusters(*this, pBestTargetCluster, pMuonCluster,
                                                                         m_targetClusterListName,
                                                                         m_muonClusterListName));
    }
  }
  ...
  if (!standaloneMuonClusters.empty()) {
    PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=,
        PandoraContentApi::SaveList(*this, m_muonClusterListName, m_targetClusterListName, standaloneMuonClusters));
  }
...
StatusCode MuonClusterAssociationAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           XmlHelper::ReadValue(xmlHandle, "MuonClusterListName", m_muonClusterListName));

  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           XmlHelper::ReadValue(xmlHandle, "TargetClusterListName", m_targetClusterListName));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "DCosCut", m_dCosCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinHitsInMuonCluster", m_minHitsInMuonCluster));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ShouldEstimateEnergyLostInCoil", m_shouldEstimateEnergyLostInCoil));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "CoilCorrectionMinInnerRadius", m_coilCorrectionMinInnerRadius));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "CoilCorrectionMinInnerLayerHits", m_coilCorrectionMinInnerLayerHits));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MinClusterHadronicEnergy", m_minClusterHadronicEnergy));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinHitsInCluster", m_minHitsInCluster));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ClusterAssociationChi", m_clusterAssociationChi));
