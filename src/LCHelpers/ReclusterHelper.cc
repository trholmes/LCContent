...
float ReclusterHelper::GetTrackClusterCompatibility(const Pandora& pandora, const Cluster* const pCluster,
...
float ReclusterHelper::GetTrackClusterCompatibility(const Pandora& pandora, const float clusterEnergy,
...
StatusCode ReclusterHelper::ExtractReclusterResults(const Pandora& pandora,
                                                    const ClusterList* const pReclusterCandidatesList,
                                                    ReclusterResult& reclusterResult) {
  unsigned int nExcessTrackAssociations(0);
  float chi(0.), chi2(0.), dof(0.), unassociatedEnergy(0.),
      minTrackAssociationEnergy(std::numeric_limits<float>::max());

  for (ClusterList::const_iterator iter = pReclusterCandidatesList->begin(), iterEnd = pReclusterCandidatesList->end();
       iter != iterEnd; ++iter) {
