...
StatusCode ClusterHelper::GetFitResultsClosestApproach(const ClusterFitResult& lhs, const ClusterFitResult& rhs,
  ...
  for (unsigned int iLayer = startLayer; iLayer <= endLayer; ++iLayer) {
    ...
    for (CaloHitList::const_iterator hitIter = iter->second->begin(), hitIterEnd = iter->second->end();
      ...
      const float distanceSquared(
          interceptDifference.GetCrossProduct(clusterFitResult.GetDirection()).GetMagnitudeSquared());
...
float ClusterHelper::GetDistanceToClosestCentroid(const ClusterFitResult& clusterFitResult,
                                                  const Cluster* const pCluster, const unsigned int startLayer,
...
StatusCode ClusterHelper::GetDistanceToClosestCentroid(const Cluster* const pClusterI, const Cluster* const pClusterJ,
                                                       float& centroidDistance) {
  // Return if clusters do not overlap
  if ((pClusterI->GetOuterPseudoLayer() < pClusterJ->GetInnerPseudoLayer()) ||
      (pClusterJ->GetOuterPseudoLayer() < pClusterI->GetInnerPseudoLayer()))
...
StatusCode ClusterHelper::GetClosestIntraLayerDistance(const Cluster* const pClusterI, const Cluster* const pClusterJ,
...
template <>
StatusCode ClusterHelper::GetTrackClusterDistance(const TrackState* const pTrackState, const Cluster* const pCluster,
  ...
  for (OrderedCaloHitList::const_iterator iter = orderedCaloHitList.begin(), iterEnd = orderedCaloHitList.end();
    ...
    for (CaloHitList::const_iterator hitIter = iter->second->begin(), hitIterEnd = iter->second->end();
      ...
      const float perpendicularDistanceSquared(
          (trackDirection.GetCrossProduct(positionDifference)).GetMagnitudeSquared());
...
StatusCode ClusterHelper::GetTrackClusterDistance(const Track* const pTrack, const Cluster* const pCluster,
                                                  const unsigned int maxSearchLayer, const float parallelDistanceCut,
                                                  const float minTrackClusterCosAngle, float& trackClusterDistance) {
  return GetTrackClusterDistance(&(pTrack->GetTrackStateAtCalorimeter()), pCluster, maxSearchLayer, parallelDistanceCut,
                                 minTrackClusterCosAngle, trackClusterDistance);
}

template <>
StatusCode ClusterHelper::GetTrackClusterDistance(const TrackStateVector* const pTrackStates,
                                                  const Cluster* const pCluster, const unsigned int maxSearchLayer,
                                                  const float parallelDistanceCut, const float minTrackClusterCosAngle,
  ...
  for (auto const& trackState : *pTrackStates) {
    ...
    const StatusCode statusCode(GetTrackClusterDistance(&trackState, pCluster, maxSearchLayer, parallelDistanceCut,
                                                        minTrackClusterCosAngle, trackClusterDistanceTemp));
...
bool ClusterHelper::CanMergeCluster(const Pandora& pandora, const Cluster* const pCluster, const float minMipFraction,
  ...
  return (pCluster->GetFitToAllHitsResult().IsFitSuccessful() &&
...
float ClusterHelper::GetEnergyWeightedMeanTime(const Cluster* const pCluster) {
  ...
  if ((energySum < std::numeric_limits<float>::epsilon()) ||
      (energyTimeProductSum < std::numeric_limits<float>::epsilon()))
...
bool ClusterHelper::DoesClusterCrossGapRegion(const Pandora& pandora, const Cluster* const pCluster,
                                              const unsigned int startLayer, const unsigned int endLayer,
  ...
  const float propagationDistance(
      (pCluster->GetCentroid(fitEndLayer) - startLayerCentroid).GetDotProduct(fitResult.GetDirection()));

  return ClusterHelper::DoesFitCrossGapRegion(pandora, fitResult, startLayerCentroid, propagationDistance,
                                              nSamplingPoints);
}

//------------------------------------------------------------------------------------------------------------------------------------------

bool ClusterHelper::DoesFitCrossGapRegion(const Pandora& pandora, const ClusterFitResult& clusterFitResult,
                                          const CartesianVector& startPosition, const float propagationDistance,
                                          const unsigned int nSamplingPoints) {
  ...
  for (unsigned int i = 0; i < nSamplingPoints; ++i) {
    const CartesianVector fitPosition(fitStartPosition +
                                      (fitPropagation * (static_cast<float>(i) / static_cast<float>(nSamplingPoints))));

    for (DetectorGapList::const_iterator iter = detectorGapList.begin(), iterEnd = detectorGapList.end();
         iter != iterEnd; ++iter) {
      // ATTN Could pass (e.g. inner layer) hit type for cluster, but know that actually need to pass only any 3D hit
      // type (hack)
...
bool ClusterHelper::IsClusterLeavingDetector(const Cluster* const pCluster, const unsigned int nOuterLayersToExamine,
                                             const unsigned int nMipLikeOccupiedLayers,
                                             const unsigned int nShowerLikeOccupiedLayers,
                                             const float showerLikeEnergyInOuterLayers) {
  ...
  if ((nOccupiedOuterLayers >= nMipLikeOccupiedLayers) ||
      ((nOccupiedOuterLayers == nShowerLikeOccupiedLayers) &&
       (hadronicEnergyInOuterLayers > showerLikeEnergyInOuterLayers))) {
    return true;
  }
...
bool ClusterHelper::ContainsHitType(const Cluster* const pCluster, const HitType hitType) {
  ...
  for (OrderedCaloHitList::const_reverse_iterator iter = orderedCaloHitList.rbegin(),
                                                  iterEnd = orderedCaloHitList.rend();
       iter != iterEnd; ++iter) {
