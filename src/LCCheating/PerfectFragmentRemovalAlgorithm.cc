...
PerfectFragmentRemovalAlgorithm::PerfectFragmentRemovalAlgorithm() : m_shouldMergeChargedClusters(false) {}
...
StatusCode PerfectFragmentRemovalAlgorithm::Run() {
  ...
  for (ClusterVector::iterator iter = chargedClusters.begin(), iterEnd = chargedClusters.end(); iter != iterEnd;
       ++iter) {
    try {
      const Cluster* const pCluster = *iter;
      ...
      if (itMCParticle == mcParticleToClusterMap.end()) {
        ...
        PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                                 PandoraContentApi::MergeAndDeleteClusters(*this, pMainChargedCluster, pCluster));
        *iter = NULL;
      }
  ...
  for (ClusterVector::iterator iter = neutralClusters.begin(), iterEnd = neutralClusters.end(); iter != iterEnd;
       ++iter) {
    try {
      const Cluster* const pCluster = *iter;
...
StatusCode PerfectFragmentRemovalAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      XmlHelper::ReadValue(xmlHandle, "ShouldMergeChargedClusters", m_shouldMergeChargedClusters));
