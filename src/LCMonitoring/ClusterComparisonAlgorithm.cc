...
ClusterComparisonAlgorithm::ClusterComparisonAlgorithm()
    : m_restoreOriginalCaloHitList(false), m_restoreOriginalTrackList(false) {}
...
void ClusterComparisonAlgorithm::TweakCurrentLists(std::string& originalCaloHitListName,
                                                   std::string& originalTrackListName) const {
  ...
  if (!m_inputTrackListName.empty()) {
    PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                            PandoraContentApi::GetCurrentListName<Track>(*this, originalTrackListName));
    PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                            PandoraContentApi::ReplaceCurrentList<Track>(*this, m_inputTrackListName));
  }
}
...
void ClusterComparisonAlgorithm::CreateAndSaveInitialClusters() const {
  ...
  PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                          PandoraContentApi::RunClusteringAlgorithm(*this, m_clusteringAlgorithmName, pTempClusterList,
                                                                    tempClusterListName));
...
void ClusterComparisonAlgorithm::RunReclustering() const {
  ...
  PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS, !=,
      PandoraContentApi::InitializeReclustering(*this, trackList, originalClusterList, newListNameForOriginalClusters));

  for (StringVector::const_iterator iter = m_comparisonAlgorithms.begin(), iterEnd = m_comparisonAlgorithms.end();
       iter != iterEnd; ++iter) {
    std::string reclusterListName;
    const ClusterList* pReclusterList(NULL);
    PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                            PandoraContentApi::RunClusteringAlgorithm(*this, *iter, pReclusterList, reclusterListName));
    ...
    if (!outputOriginalClusters.empty() || !outputAlternativeClusters.empty()) {
      std::cout << "nRelevant original clusters: " << outputOriginalClusters.size()
                << ", nRelevant alternative clusters: " << outputAlternativeClusters.size() << std::endl;
      ...
      PANDORA_MONITORING_API(
          VisualizeClusters(this->GetPandora(), &outputAlternativeClusters, "Alternative Clusters", RED));
      PANDORA_MONITORING_API(ViewEvent(this->GetPandora()));
    }
  }

  PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                          PandoraContentApi::EndReclustering(*this, newListNameForOriginalClusters));
}

//------------------------------------------------------------------------------------------------------------------------------------------

void ClusterComparisonAlgorithm::CompareClusters(const ClusterList& clusterList1, const ClusterList& clusterList2,
                                                 ClusterList& outputList1, ClusterList& outputList2) const {
  ...
  for (const auto& mapEntry : clusterToHitListMap1)
  ...
  for (const Cluster* const pCluster1 : clusterList) {
    ...
    for (CaloHitList::const_iterator hIter = caloHitList1.begin(), hIterEnd = caloHitList1.end(); hIter != hIterEnd;
         ++hIter) {
      HitToClusterMap::const_iterator htcIter2 = hitToClusterMap2.find(*hIter);
      (hitToClusterMap2.end() != htcIter2) ? linkedClusterList2.push_back(htcIter2->second)
                                           : outputList1.push_back(pCluster1);
    }

    // Check how many clusters in list 2 share hits with the single cluster in list 1; if just one, compare total
    // numbers of hits
...
void ClusterComparisonAlgorithm::PopulateMaps(const pandora::ClusterList& clusterList,
                                              ClusterToHitListMap& clusterToHitListMap,
  ...
  for (ClusterList::const_iterator iter = clusterList.begin(), iterEnd = clusterList.end(); iter != iterEnd; ++iter) {
    ...
    for (CaloHitList::const_iterator hIter = caloHitList.begin(), hIterEnd = caloHitList.end(); hIter != hIterEnd;
         ++hIter) {
...
void ClusterComparisonAlgorithm::RestoreInputLists(const std::string& originalCaloHitListName,
  ...
  if (m_restoreOriginalTrackList && !m_inputTrackListName.empty())
    PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                            PandoraContentApi::ReplaceCurrentList<Track>(*this, originalTrackListName));
}
...
StatusCode ClusterComparisonAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      XmlHelper::ReadValue(xmlHandle, "RestoreOriginalCaloHitList", m_restoreOriginalCaloHitList));

  m_restoreOriginalTrackList = !m_inputTrackListName.empty();
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "RestoreOriginalTrackList", m_restoreOriginalTrackList));

  PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=,
      XmlHelper::ProcessAlgorithm(*this, xmlHandle, "OriginalClusters", m_clusteringAlgorithmName));

  PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=,
      XmlHelper::ProcessAlgorithmList(*this, xmlHandle, "ComparisonAlgorithms", m_comparisonAlgorithms));
