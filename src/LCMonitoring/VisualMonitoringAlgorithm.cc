...
VisualMonitoringAlgorithm::VisualMonitoringAlgorithm()
    : m_showCurrentMCParticles(false), m_showCurrentCaloHits(false), m_showCurrentTracks(false),
      m_showCurrentClusters(true), m_showCurrentPfos(true), m_showCurrentVertices(false), m_displayEvent(true),
      m_showDetector(true), m_detectorView("default"), m_showOnlyAvailable(false), m_showAssociatedTracks(false),
      m_hitColors("pfo"), m_thresholdEnergy(-1.f), m_transparencyThresholdE(-1.f), m_energyScaleThresholdE(1.f),
      m_scalingFactor(0.1f), m_showPfoVertices(true), m_showPfoHierarchy(true) {}
...
StatusCode VisualMonitoringAlgorithm::Run() {
  PANDORA_MONITORING_API(SetEveDisplayParameters(this->GetPandora(), m_showDetector,
                                                 ...
                                                 : (m_detectorView.find("xy") != std::string::npos)
                                                     ? DETECTOR_VIEW_XY
  ...
  for (StringVector::const_iterator iter = m_trackListNames.begin(), iterEnd = m_trackListNames.end(); iter != iterEnd;
       ++iter) {
...
void VisualMonitoringAlgorithm::VisualizeMCParticleList(const std::string& listName) const {
  ...
  PANDORA_MONITORING_API(VisualizeMCParticles(this->GetPandora(), pMCParticleList,
                                              listName.empty() ? "CurrentMCParticles" : listName.c_str(), AUTO,
...
void VisualMonitoringAlgorithm::VisualizeCaloHitList(const std::string& listName) const {
  ...
  PANDORA_MONITORING_API(VisualizeCaloHits(this->GetPandora(), &caloHitList,
...
void VisualMonitoringAlgorithm::VisualizeTrackList(const std::string& listName) const {
  ...
  PANDORA_MONITORING_API(
      VisualizeTracks(this->GetPandora(), &trackList, listName.empty() ? "CurrentTracks" : listName.c_str(), GRAY));
}
...
void VisualMonitoringAlgorithm::VisualizeClusterList(const std::string& listName) const {
  ...
  PANDORA_MONITORING_API(VisualizeClusters(this->GetPandora(), &clusterList,
...
void VisualMonitoringAlgorithm::VisualizeParticleFlowList(const std::string& listName) const {
  ...
  PANDORA_MONITORING_API(
      VisualizeParticleFlowObjects(this->GetPandora(), pPfoList, listName.empty() ? "CurrentPfos" : listName.c_str(),
...
void VisualMonitoringAlgorithm::VisualizeVertexList(const std::string& listName) const {
  ...
  PANDORA_MONITORING_API(VisualizeVertices(this->GetPandora(), &vertexList,
                                           listName.empty() ? "CurrentVertices" : listName.c_str(), AUTO));
}
...
StatusCode VisualMonitoringAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  for (StringVector::iterator iter = m_suppressMCParticles.begin(), iterEnd = m_suppressMCParticles.end();
       iter != iterEnd; ++iter) {
