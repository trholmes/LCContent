...
EfficiencyMonitoringAlgorithm::EfficiencyMonitoringAlgorithm() : m_mcThresholdEnergy(0.f) {}
...
EfficiencyMonitoringAlgorithm::~EfficiencyMonitoringAlgorithm() {
  try {
    ...
    std::cout << "EfficiencyMonitoringAlgorithm: Unable to write tree EffTree to file " << m_monitoringFileName
              << std::endl;
  }
}
...
StatusCode EfficiencyMonitoringAlgorithm::Run() {
  ...
  if (1 != pMCParticleList->size()) {
    std::cout << "EfficiencyMonitoring - Algorithm works only with single particle samples, nParticles "
              << pMCParticleList->size() << std::endl;
  ...
  if (mcEnergy < m_mcThresholdEnergy) {
    std::cout << "EfficiencyMonitoring - MC particle energy below threshold " << mcEnergy << "( < "
              << m_mcThresholdEnergy << ")" << std::endl;
    return STATUS_CODE_SUCCESS;
  }
...
StatusCode EfficiencyMonitoringAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           XmlHelper::ReadValue(xmlHandle, "MonitoringFileName", m_monitoringFileName));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MCThresholdEnergy", m_mcThresholdEnergy));
