...
DumpPfosMonitoringAlgorithm::DumpPfosMonitoringAlgorithm()
    : m_trackRecoAsTrackEnergy(0.f), m_trackRecoAsPhotonEnergy(0.f), m_trackRecoAsNeutralEnergy(0.f),
      m_photonRecoAsTrackEnergy(0.f), m_photonRecoAsPhotonEnergy(0.f), m_photonRecoAsNeutralEnergy(0.f),
      m_neutralRecoAsTrackEnergy(0.f), m_neutralRecoAsPhotonEnergy(0.f), m_neutralRecoAsNeutralEnergy(0.f), m_count(0),
      m_photonOrNeutralRecoAsTrackEnergySum(0.f), m_photonOrNeutralRecoAsTrackEnergySum2(0.f),
      m_trackRecoAsPhotonOrNeutralEnergySum(0.f), m_trackRecoAsPhotonOrNeutralEnergySum2(0.f),
      m_confusionCorrelation(0.f), m_minPfoEnergyToDisplay(0.f), m_minAbsChiToDisplay(3.f),
      m_minConfusionEnergyToDisplay(5.f), m_minFragmentEnergyToDisplay(5.f), m_totalPfoEnergyDisplayLessThan(1000000.f),
      m_totalPfoEnergyDisplayGreaterThan(0.f), m_fragmentEnergyToDisplay(5.f), m_photonIdEnergyToDisplay(5.f),
      m_trackRecoAsTrackEnergySum(0.f), m_trackRecoAsPhotonEnergySum(0.f), m_trackRecoAsNeutralEnergySum(0.f),
      m_photonRecoAsTrackEnergySum(0.f), m_photonRecoAsPhotonEnergySum(0.f), m_photonRecoAsNeutralEnergySum(0.f),
      m_neutralRecoAsTrackEnergySum(0.f), m_neutralRecoAsPhotonEnergySum(0.f), m_neutralRecoAsNeutralEnergySum(0.f),
      m_goodTrackEnergy(0.f), m_goodPhotonEnergy(0.f), m_goodIdedPhotonEnergy(0.f), m_goodNeutralEnergy(0.f),
      m_goodIdedNeutralEnergy(0.f), m_badTrackEnergy(0.f), m_badPhotonEnergy(0.f), m_badNeutralEnergy(0.f),
      m_goodTrackEnergySum(0.f), m_goodPhotonEnergySum(0.f), m_goodIdedPhotonEnergySum(0.f),
      m_goodNeutralEnergySum(0.f), m_goodIdedNeutralEnergySum(0.f), m_badTrackEnergySum(0.f), m_badPhotonEnergySum(0.f),
      m_badNeutralEnergySum(0.f), m_goodFractionCut(0.9f), m_firstChargedPfoToPrint(true),
      m_firstNeutralPfoToPrint(true), m_firstPhotonPfoToPrint(true) {}
...
DumpPfosMonitoringAlgorithm::~DumpPfosMonitoringAlgorithm() {
  ...
  std::cout << std::endl;
  FORMATTED_OUTPUT_CONFUSION(confmat[0][0], confmat[0][1], confmat[0][2], confmat[1][0], confmat[1][1], confmat[1][2],
  ...
  if (m_count > 0) {
    ...
    const float sigmaA =
        std::sqrt(m_photonOrNeutralRecoAsTrackEnergySum2 / static_cast<float>(m_count) - confusionA * confusionA);
    const float sigmaB =
        std::sqrt(m_trackRecoAsPhotonOrNeutralEnergySum2 / static_cast<float>(m_count) - confusionB * confusionB);
    ...
    std::cout << " Rms total confusion                          : "
              << std::sqrt(sigmaA * sigmaA + sigmaB * sigmaB + 2 * rhoAB * sigmaA * sigmaB) << std::endl;
    std::cout << " Rms net confusion                            : "
              << std::sqrt(sigmaA * sigmaA + sigmaB * sigmaB - 2 * rhoAB * sigmaA * sigmaB) << std::endl;
    std::cout << std::endl;
    ...
    if (nSumCal > std::numeric_limits<float>::epsilon()) {
      const float econfmat[3][3] = {{m_trackRecoAsTrackEnergySum / nSumCal, m_photonRecoAsTrackEnergySum / nSumCal,
                                     m_neutralRecoAsTrackEnergySum / nSumCal},
                                    {m_trackRecoAsPhotonEnergySum / nSumCal, m_photonRecoAsPhotonEnergySum / nSumCal,
                                     m_neutralRecoAsPhotonEnergySum / nSumCal},
                                    {m_trackRecoAsNeutralEnergySum / nSumCal, m_photonRecoAsNeutralEnergySum / nSumCal,
                                     m_neutralRecoAsNeutralEnergySum / nSumCal}};

      FORMATTED_OUTPUT_CONFUSION(econfmat[0][0], econfmat[0][1], econfmat[0][2], econfmat[1][0], econfmat[1][1],
                                 econfmat[1][2], econfmat[2][0], econfmat[2][1], econfmat[2][2]);
      ...
      if (m_goodPhotonEnergySum > std::numeric_limits<float>::epsilon())
        std::cout << " Correctly IDed Photon  Energy Fraction : " << m_goodIdedPhotonEnergySum / m_goodPhotonEnergySum
                  << std::endl;

      if (m_goodNeutralEnergySum > std::numeric_limits<float>::epsilon())
        std::cout << " Correctly IDed Neutral Energy Fraction : " << m_goodIdedNeutralEnergySum / m_goodNeutralEnergySum
                  << std::endl;
    }
  }
...
StatusCode DumpPfosMonitoringAlgorithm::Run() {
  ...
  if ((totalPfoEnergy > m_totalPfoEnergyDisplayGreaterThan) && (totalPfoEnergy < m_totalPfoEnergyDisplayLessThan)) {
    ...
    for (ParticleFlowObjectVector::const_iterator pfoIter = sortedChargedPfos.begin();
         pfoIter != sortedChargedPfos.end(); ++pfoIter) {
      this->DumpChargedPfo(*pfoIter);
    }

    for (ParticleFlowObjectVector::const_iterator pfoIter = sortedPhotonPfos.begin(); pfoIter != sortedPhotonPfos.end();
         ++pfoIter) {
      this->DumpPhotonPfo(*pfoIter);
    }

    for (ParticleFlowObjectVector::const_iterator pfoIter = sortedNeutralHadronPfos.begin();
         pfoIter != sortedNeutralHadronPfos.end(); ++pfoIter) {
    ...
    if (nSumCal > 0.f) {
      ...
      const float econfmat[3][3] = {{m_trackRecoAsTrackEnergy / nSumCal, m_photonRecoAsTrackEnergy / nSumCal,
                                     m_neutralRecoAsTrackEnergy / nSumCal},
                                    {m_trackRecoAsPhotonEnergy / nSumCal, m_photonRecoAsPhotonEnergy / nSumCal,
                                     m_neutralRecoAsPhotonEnergy / nSumCal},
                                    {m_trackRecoAsNeutralEnergy / nSumCal, m_photonRecoAsNeutralEnergy / nSumCal,
                                     m_neutralRecoAsNeutralEnergy / nSumCal}};
...
void DumpPfosMonitoringAlgorithm::DumpNeutralOrPhotonPfo(const ParticleFlowObject* const pPfo, bool isPhotonPfo) {
  ...
  for (ClusterList::const_iterator clusterIter = clusterList.begin(); clusterIter != clusterList.end(); ++clusterIter) {
    ...
    const bool badPhotonId(
        (!isPhotonPfo && (fPhoton > 0.8f) && ((clusterEnergy * fPhoton) > m_photonIdEnergyToDisplay)) ||
    ...
    if (printThisPfo) {
      ...
      FORMATTED_OUTPUT_NEUTRAL(
          clusterEnergy, clusterTime, fCharged, fPhoton, fNeutral, pCluster->GetInnerPseudoLayer(),
...
void DumpPfosMonitoringAlgorithm::ClusterEnergyFractions(const Cluster* const pCluster, float& fCharged, float& fPhoton,
                                                         float& fNeutral, const MCParticle*& pBestMatchedMCPfo) const {
      ...
      try {
        ...
        if ((charge != 0) || (std::abs(pdgCode) == LAMBDA) || (std::abs(pdgCode) == K_SHORT)) {
          if (m_trackMcPfoTargets.end() ==
              std::find(m_trackMcPfoTargets.begin(), m_trackMcPfoTargets.end(), pMCParticle)) {
            neutralEnergy += pCaloHit->GetHadronicEnergy();
          } else {
            chargedEnergy += pCaloHit->GetHadronicEnergy();
          }
        } else {
          (pMCParticle->GetParticleId() == PHOTON) ? photonEnergy += pCaloHit->GetHadronicEnergy()
                                                   : neutralEnergy += pCaloHit->GetHadronicEnergy();
        }
      } catch (StatusCodeException&) {
      }
...
StatusCode DumpPfosMonitoringAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinPfoEnergyToDisplay", m_minPfoEnergyToDisplay));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinAbsChiToDisplay", m_minAbsChiToDisplay));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MinConfusionEnergyToDisplay", m_minConfusionEnergyToDisplay));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MinFragmentEnergyToDisplay", m_minFragmentEnergyToDisplay));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "TotalPfoEnergyDisplayLessThan", m_totalPfoEnergyDisplayLessThan));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "TotalPfoEnergyDisplayGreaterThan", m_totalPfoEnergyDisplayGreaterThan));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "FragmentEnergyToDisplay", m_fragmentEnergyToDisplay));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "PhotonIdEnergyToDisplay", m_photonIdEnergyToDisplay));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "GoodFractionCut", m_goodFractionCut));
