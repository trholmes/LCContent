...
TrackRecoveryInteractionsAlgorithm::TrackRecoveryInteractionsAlgorithm()
    : m_maxTrackClusterDistance(200.f), m_trackClusterDistanceCut(100.f), m_clusterEnergyCut(0.5f),
      m_directionCosineCut(0.9f), m_maxTrackAssociationChi(2.f), m_maxSearchLayer(19), m_parallelDistanceCut(100.f),
...
StatusCode TrackRecoveryInteractionsAlgorithm::Run() {
  ...
  for (TrackVector::const_iterator iterT = trackVector.begin(), iterTEnd = trackVector.end(); iterT != iterTEnd;
    ...
    if ((smallestTrackClusterDistance > m_trackClusterDistanceCut) && (clusterEnergy > m_clusterEnergyCut)) {
      ...
      const CartesianVector trackerToTrackCalorimeterUnitVector(
          (trackCalorimeterPosition - trackerEndPosition).GetUnitVector());
...
StatusCode TrackRecoveryInteractionsAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "TrackClusterDistanceCut", m_trackClusterDistanceCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ClusterEnergyCut", m_clusterEnergyCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "DirectionCosineCut", m_directionCosineCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxTrackAssociationChi", m_maxTrackAssociationChi));
