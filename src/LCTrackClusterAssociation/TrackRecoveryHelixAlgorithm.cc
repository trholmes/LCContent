...
TrackRecoveryHelixAlgorithm::TrackRecoveryHelixAlgorithm()
    : m_maxTrackClusterDeltaZ(250.f), m_maxAbsoluteTrackClusterChi(2.5f), m_maxLayersCrossed(50), m_maxSearchLayer(19),
      m_parallelDistanceCut(100.f), m_minTrackClusterCosAngle(0.f), m_helixComparisonNLayers(20),
      m_helixComparisonMaxOccupiedLayers(9), m_maxTrackClusterDistance(100.f), m_maxClosestHelixClusterDistance(100.f),
...
StatusCode
TrackRecoveryHelixAlgorithm::GetTrackAssociationInfoMap(TrackAssociationInfoMap& trackAssociationInfoMap) const {
  ...
  for (TrackList::const_iterator iterT = pTrackList->begin(), iterTEnd = pTrackList->end(); iterT != iterTEnd;
    ...
    for (ClusterList::const_iterator iterC = pClusterList->begin(), iterCEnd = pClusterList->end(); iterC != iterCEnd;
      ...
      if (!pCluster->GetAssociatedTrackList().empty() || (0 == pCluster->GetNCaloHits()) ||
          pCluster->PassPhotonId(this->GetPandora()))
        continue;
      ...
      const unsigned int nLayersCrossed(FragmentRemovalHelper::GetNLayersCrossed(
          this->GetPandora(), helix, trackCalorimeterZPosition, clusterZPosition));
      ...
      if (STATUS_CODE_SUCCESS !=
          ClusterHelper::GetTrackClusterDistance(pTrack, pCluster, m_maxSearchLayer, m_parallelDistanceCut,
      ...
      if (STATUS_CODE_SUCCESS != FragmentRemovalHelper::GetClusterHelixDistance(
                                     pCluster, helix, innerLayer, innerLayer + m_helixComparisonNLayers,
                                     m_helixComparisonMaxOccupiedLayers, closestDistanceToHit, meanDistanceToHits)) {
      ...
      if ((trackClusterDistance > m_maxTrackClusterDistance) &&
          ((closestDistanceToHit > m_maxClosestHelixClusterDistance) ||
           (meanDistanceToHits > m_maxMeanHelixClusterDistance))) {
        continue;
      }
...
StatusCode
TrackRecoveryHelixAlgorithm::MakeTrackClusterAssociations(TrackAssociationInfoMap& trackAssociationInfoMap) const {
  ...
  while (shouldContinue) {
    ...
    TrackList trackList;
    for (const auto& mapEntry : trackAssociationInfoMap)
      trackList.push_back(mapEntry.first);
    ...
    for (const Track* const pTrack : trackList) {
      ...
      for (const AssociationInfo& associationInfo : associationInfoSet) {
        ...
        const float energyDifference(
            std::fabs(associationInfo.GetCluster()->GetHadronicEnergy() - pTrack->GetEnergyAtDca()));

        if ((approach < closestApproach) ||
            ((approach == closestApproach) && (energyDifference < minEnergyDifference))) {
    ...
    if ((NULL != pBestTrack) && (NULL != pBestCluster)) {
      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                               PandoraContentApi::AddTrackClusterAssociation(*this, pBestTrack, pBestCluster));

      // Clear information to prevent multiple associations to same track/cluster
      trackAssociationInfoMap.erase(pBestTrack);

      for (TrackAssociationInfoMap::iterator iter = trackAssociationInfoMap.begin(),
                                             iterEnd = trackAssociationInfoMap.end();
           iter != iterEnd; ++iter) {
        for (AssociationInfoSet::iterator infoIter = iter->second.begin(), infoIterEnd = iter->second.end();
             infoIter != infoIterEnd;) {
...
bool TrackRecoveryHelixAlgorithm::AssociationInfo::operator<(
    const TrackRecoveryHelixAlgorithm::AssociationInfo& rhs) const {
...
StatusCode TrackRecoveryHelixAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxLayersCrossed", m_maxLayersCrossed));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "HelixComparisonNLayers", m_helixComparisonNLayers));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "HelixComparisonMaxOccupiedLayers", m_helixComparisonMaxOccupiedLayers));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MaxClosestHelixClusterDistance", m_maxClosestHelixClusterDistance));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MaxMeanHelixClusterDistance", m_maxMeanHelixClusterDistance));
