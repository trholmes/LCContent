...
TrackRecoveryAlgorithm::TrackRecoveryAlgorithm()
    : m_maxTrackZStart(100.f), m_maxAbsoluteTrackClusterChi(2.f), m_endCapMaxTrackClusterDistance1(100.f),
      m_endCapMaxTrackClusterDistance2(250.f), m_barrelMaxTrackClusterDistance(10.f), m_maxSearchLayer(19),
      m_parallelDistanceCut(100.f), m_minTrackClusterCosAngle(0.f) {}
  ...
  for (TrackVector::const_iterator iterT = trackVector.begin(), iterTEnd = trackVector.end(); iterT != iterTEnd;
    ...
    if (pTrack->IsProjectedToEndCap()) {
      if ((smallestTrackClusterDistance < m_endCapMaxTrackClusterDistance1) ||
          ((smallestTrackClusterDistance < m_endCapMaxTrackClusterDistance2) &&
           (pBestCluster->GetTrackComparisonEnergy(this->GetPandora()) < trackEnergy))) {
...
StatusCode TrackRecoveryAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxTrackZStart", m_maxTrackZStart));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "EndCapMaxTrackClusterDistance1", m_endCapMaxTrackClusterDistance1));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "EndCapMaxTrackClusterDistance2", m_endCapMaxTrackClusterDistance2));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "BarrelMaxTrackClusterDistance", m_barrelMaxTrackClusterDistance));
