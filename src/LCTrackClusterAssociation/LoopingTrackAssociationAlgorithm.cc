...
LoopingTrackAssociationAlgorithm::LoopingTrackAssociationAlgorithm()
    : m_maxTrackClusterDeltaZ(50.f), m_minHitsInCluster(4), m_minOccupiedLayersInCluster(4), m_maxClusterInnerLayer(9),
      m_maxAbsoluteTrackClusterChi(2.f), m_maxDeltaR(50.f), m_minDeltaR(-100.f), m_nClusterFitLayers(10),
      m_nClusterDeltaRLayers(9), m_directionCosineCut(0.975f), m_clusterMipFractionCut(0.5f),
      m_directionCosineCut1(0.925f), m_directionCosineCut2(0.85f), m_deltaRCut2(50.f), m_directionCosineCut3(0.75f),
      m_deltaRCut3(25.f), m_directionCosineCut4(0.f), m_deltaRCut4(10.f) {}
...
StatusCode LoopingTrackAssociationAlgorithm::Run() {
  ...
  for (TrackVector::const_iterator iterT = trackVector.begin(), iterTEnd = trackVector.end(); iterT != iterTEnd;
       ++iterT) {
    ...
    for (ClusterList::const_iterator iterC = pClusterList->begin(), iterCEnd = pClusterList->end(); iterC != iterCEnd;
         ++iterC) {
      ...
      const float chi(ReclusterHelper::GetTrackClusterCompatibility(
          this->GetPandora(), pCluster->GetTrackComparisonEnergy(this->GetPandora()), trackEnergy));

      if (std::fabs(chi) > m_maxAbsoluteTrackClusterChi)
        continue;

      // Calculate distance of cluster from centre of helix for i) cluster inner layer and ii) first
      // m_nClusterDeltaRLayers layers
      ...
      const float innerLayerDeltaR(
          std::sqrt((innerLayerDeltaX * innerLayerDeltaX) + (innerLayerDeltaY * innerLayerDeltaY)) -
          std::fabs(helixRadius));
      ...
      const float deltaR(
          std::min(std::fabs(innerLayerDeltaR), std::fabs(meanDeltaR))); // ATTN: Changed order of min and fabs here

      // Calculate projected helix direction at endcap
      CartesianVector helixDirection(0.f, 0.f, 0.f);

      if (0.f != innerLayerDeltaY) {
        float helixDCosX((1.f - helixDCosZ * helixDCosZ) /
                         (1.f + ((innerLayerDeltaX * innerLayerDeltaX) / (innerLayerDeltaY * innerLayerDeltaY))));
    ...
    if (NULL != pBestCluster) {
      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                               PandoraContentApi::AddTrackClusterAssociation(*this, pTrack, pBestCluster));
    }
  }
...
float LoopingTrackAssociationAlgorithm::GetMeanDeltaR(const Cluster* const pCluster, const float helixXCentre,
                                                      const float helixYCentre, const float helixRadius) const {
  float deltaRSum(0.f);
...
StatusCode LoopingTrackAssociationAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxTrackClusterDeltaZ", m_maxTrackClusterDeltaZ));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxClusterInnerLayer", m_maxClusterInnerLayer));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MaxAbsoluteTrackClusterChi", m_maxAbsoluteTrackClusterChi));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxDeltaR", m_maxDeltaR));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinDeltaR", m_minDeltaR));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "NClusterFitLayers", m_nClusterFitLayers));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "NClusterDeltaRLayers", m_nClusterDeltaRLayers));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "DirectionCosineCut", m_directionCosineCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ClusterMipFractionCut", m_clusterMipFractionCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "DirectionCosineCut1", m_directionCosineCut1));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "DirectionCosineCut2", m_directionCosineCut2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "DeltaRCut2", m_deltaRCut2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "DirectionCosineCut3", m_directionCosineCut3));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "DeltaRCut3", m_deltaRCut3));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "DirectionCosineCut4", m_directionCosineCut4));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "DeltaRCut4", m_deltaRCut4));
