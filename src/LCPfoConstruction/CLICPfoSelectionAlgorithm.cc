...
CLICPfoSelectionAlgorithm::CLICPfoSelectionAlgorithm()
    : m_monitoring(false), m_displaySelectedPfos(false), m_displayRejectedPfos(true),
      m_monitoringPfoEnergyToDisplay(1.f), m_farForwardCosTheta(0.975f), m_ptCutForTightTiming(0.75f),
      m_photonPtCut(0.f), m_photonPtCutForLooseTiming(4.f), m_photonLooseTimingCut(2.f), m_photonTightTimingCut(1.f),
      m_chargedPfoPtCut(0.f), m_chargedPfoPtCutForLooseTiming(4.f), m_chargedPfoLooseTimingCut(3.f),
      m_chargedPfoTightTimingCut(1.5f), m_chargedPfoNegativeLooseTimingCut(-2.f),
      m_chargedPfoNegativeTightTimingCut(-2.f), m_neutralHadronPtCut(0.f), m_neutralHadronPtCutForLooseTiming(8.f),
      m_neutralHadronLooseTimingCut(2.5f), m_neutralHadronTightTimingCut(1.5f), m_neutralFarForwardLooseTimingCut(2.f),
      m_neutralFarForwardTightTimingCut(1.f), m_hCalBarrelLooseTimingCut(20.f), m_hCalBarrelTightTimingCut(10.f),
      m_hCalEndCapTimingFactor(1.f), m_neutralHadronBarrelPtCutForLooseTiming(3.5f), m_minECalHitsForTiming(5),
      m_minHCalEndCapHitsForTiming(5), m_useClusterLessPfos(true), m_minMomentumForClusterLessPfos(0.5f) {}
    ...
    if (!pfoClusterList.empty()) {
      for (ClusterList::const_iterator clusterIter = pfoClusterList.begin(), clusterIterEnd = pfoClusterList.end();
        ...
        if (nECalHits > m_minECalHitsForTiming) {
          ...
          if ((meanTimeHCalEndCap >= timingCutLow) &&
              (meanTimeHCalEndCap <= (m_hCalEndCapTimingFactor * timingCutHigh)))
            selectPfo = true;
        } else {
        ...
        if (selectPfo && (pPfo->GetEnergy() > pfoEnergyToDisplay) && m_displaySelectedPfos) {
          std::cout << " SELECTED PFO " << pPfo->GetParticleId() << " tracks = " << trackList.size()
                    << " e = " << pPfo->GetEnergy() << " pt = " << pfoPt << " nc = " << pfoClusterList.size()
                    << " t = " << meanTime << " ne = " << nECalHits << " te = " << meanTimeECal
                    << " nhe = " << nHCalEndCapHits << " the = " << meanTimeHCalEndCap << std::endl;
        }

        if (!selectPfo && (pPfo->GetEnergy() > pfoEnergyToDisplay) && m_displayRejectedPfos) {
          std::cout << " REJECTED PFO " << pPfo->GetParticleId() << " tracks = " << trackList.size()
                    << " e = " << pPfo->GetEnergy() << " pt = " << pfoPt << " nc = " << pfoClusterList.size()
                    << " t = " << meanTime << " ne = " << nECalHits << " te = " << meanTimeECal
                    << " nhe = " << nHCalEndCapHits << " the = " << meanTimeHCalEndCap << std::endl;
        }
...
void CLICPfoSelectionAlgorithm::GetClusterTimes(const Cluster* const pCluster, float& meanTime, float& meanTimeECal,
                                                unsigned int& nECalHits, float& meanTimeHCalEndCap,
                                                unsigned int& nHCalEndCapHits) const {
...
StatusCode CLICPfoSelectionAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "Monitoring", m_monitoring));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "DisplaySelectedPfos", m_displaySelectedPfos));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "DisplayRejectedPfos", m_displayRejectedPfos));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MonitoringPfoEnergyToDisplay", m_monitoringPfoEnergyToDisplay));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "FarForwardCosTheta", m_farForwardCosTheta));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "PtCutForTightTiming", m_ptCutForTightTiming));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "PhotonPtCut", m_photonPtCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "PhotonPtCutForLooseTiming", m_photonPtCutForLooseTiming));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "PhotonLooseTimingCut", m_photonLooseTimingCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "PhotonTightTimingCut", m_photonTightTimingCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ChargedPfoPtCut", m_chargedPfoPtCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ChargedPfoPtCutForLooseTiming", m_chargedPfoPtCutForLooseTiming));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ChargedPfoLooseTimingCut", m_chargedPfoLooseTimingCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ChargedPfoTightTimingCut", m_chargedPfoTightTimingCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ChargedPfoNegativeLooseTimingCut", m_chargedPfoNegativeLooseTimingCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ChargedPfoNegativeTightTimingCut", m_chargedPfoNegativeTightTimingCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "NeutralHadronPtCut", m_neutralHadronPtCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "NeutralHadronPtCutForLooseTiming", m_neutralHadronPtCutForLooseTiming));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "NeutralHadronLooseTimingCut", m_neutralHadronLooseTimingCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "NeutralHadronTightTimingCut", m_neutralHadronTightTimingCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "NeutralFarForwardLooseTimingCut", m_neutralFarForwardLooseTimingCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "NeutralFarForwardTightTimingCut", m_neutralFarForwardTightTimingCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "HCalBarrelLooseTimingCut", m_hCalBarrelLooseTimingCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "HCalBarrelTightTimingCut", m_hCalBarrelTightTimingCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "HCalEndCapTimingFactor", m_hCalBarrelLooseTimingCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "NeutralHadronBarrelPtCutForLooseTiming",
                                                       m_neutralHadronBarrelPtCutForLooseTiming));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinECalHitsForTiming", m_minECalHitsForTiming));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MinHCalEndCapHitsForTiming", m_minHCalEndCapHitsForTiming));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "UseClusterLessPfos", m_useClusterLessPfos));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MinMomentumForClusterLessPfos", m_minMomentumForClusterLessPfos));
