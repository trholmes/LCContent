...
V0PfoCreationAlgorithm::V0PfoCreationAlgorithm() : m_mcMonitoring(false) {}
...
StatusCode V0PfoCreationAlgorithm::Run() {
  ...
  for (PfoList::const_iterator iter = pPfoList->begin(), iterEnd = pPfoList->end(); iter != iterEnd; ++iter) {
    ...
    if (std::abs(pTrack1->GetParticleId()) == std::abs(E_MINUS) &&
        std::abs(pTrack2->GetParticleId()) == std::abs(E_MINUS)) {
    ...
    if (std::abs(pTrack1->GetParticleId()) == std::abs(PI_PLUS) &&
        std::abs(pTrack2->GetParticleId()) == std::abs(PI_PLUS)) {
      if (STATUS_CODE_SUCCESS == this->GetV0Mass(momentumAtStart1, momentumAtStart2,
                                                 PdgTable::GetParticleMass(PI_MINUS),
    ...
    if (std::abs(pTrack1->GetParticleId()) == std::abs(PROTON) &&
        std::abs(pTrack2->GetParticleId()) == std::abs(PI_MINUS)) {
    ...
    if (std::abs(pTrack1->GetParticleId()) == std::abs(PI_MINUS) &&
        std::abs(pTrack2->GetParticleId()) == std::abs(PROTON)) {
      if (STATUS_CODE_SUCCESS == this->GetV0Mass(momentumAtStart1, momentumAtStart2,
                                                 PdgTable::GetParticleMass(PI_MINUS), PdgTable::GetParticleMass(PROTON),
                                                 energy, mass)) {
        PandoraContentApi::ParticleFlowObject::Metadata metadata;
    ...
    if (m_mcMonitoring) {
      ...
      try {
        ...
        for (MCParticleList::const_iterator pIter = parentList1.begin(), pIterEnd = parentList1.end();
        ...
        for (MCParticleList::const_iterator pIter = parentList2.begin(), pIterEnd = parentList2.end();
             pIter != pIterEnd; ++pIter) {
          int dipdg = (*pIter)->GetParticleId();
          const float energyD = (*pIter)->GetEnergy();
      ...
      if (goodV0) {
        std::cout << " Good V0 : " << pPfo->GetParticleId() << " " << pTrack1->GetEnergyAtDca() << " "
                  << pTrack2->GetEnergyAtDca() << " eID " << isElectron1 << " " << isElectron2 << " Mass = " << mass
                  << std::endl;
      } else {
...
StatusCode V0PfoCreationAlgorithm::GetV0Mass(const CartesianVector& momentum1, const CartesianVector& momentum2,
...
StatusCode V0PfoCreationAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "McMonitoring", m_mcMonitoring));
