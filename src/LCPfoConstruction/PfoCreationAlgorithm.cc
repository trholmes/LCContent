...
PfoCreationAlgorithm::PfoCreationAlgorithm()
    : m_shouldCreateTrackBasedPfos(true), m_shouldCreateNeutralPfos(true), m_minClusterHadronicEnergy(0.25f),
      m_minClusterElectromagneticEnergy(0.f), m_minHitsInCluster(5), m_allowSingleLayerClusters(false),
...
StatusCode PfoCreationAlgorithm::PopulateTrackBasedPfo(const Track* const pTrack, PfoParameters& pfoParameters,
                                                       const bool readSiblingInfo) const {
  // Add track to the pfo
  if (pfoParameters.m_trackList.end() !=
      std::find(pfoParameters.m_trackList.begin(), pfoParameters.m_trackList.end(), pTrack))
    return STATUS_CODE_SUCCESS;
  ...
  if (readSiblingInfo) {
    ...
    for (TrackList::const_iterator iter = siblingTrackList.begin(), iterEnd = siblingTrackList.end(); iter != iterEnd;
         ++iter) {
  ...
  for (TrackList::const_iterator iter = daughterTrackList.begin(), iterEnd = daughterTrackList.end(); iter != iterEnd;
       ++iter) {
...
StatusCode PfoCreationAlgorithm::SetTrackBasedPfoParameters(const Track* const pTrack,
...
StatusCode PfoCreationAlgorithm::SetSiblingTrackBasedPfoParameters(const Track* const pTrack,
                                                                   PfoParameters& pfoParameters) const {
  int charge(0);
  float energy(0.f);
  ...
  for (TrackList::const_iterator iter = fullSiblingTrackList.begin(), iterEnd = fullSiblingTrackList.end();
       iter != iterEnd; ++iter) {
...
StatusCode PfoCreationAlgorithm::SetDaughterTrackBasedPfoParameters(const Track* const pTrack,
                                                                    PfoParameters& pfoParameters) const {
...
StatusCode PfoCreationAlgorithm::SetSimpleTrackBasedPfoParameters(const Track* const pTrack,
                                                                  PfoParameters& pfoParameters) const {
...
StatusCode PfoCreationAlgorithm::CreateNeutralPfos() const {
  ...
  for (ClusterList::const_iterator iter = pClusterList->begin(), iterEnd = pClusterList->end(); iter != iterEnd;
    ...
    float clusterEnergy(isPhoton ? pCluster->GetCorrectedElectromagneticEnergy(this->GetPandora())
    ...
    // Photon position: 0) unweighted inner centroid, 1) energy-weighted inner centroid, 2+) energy-weighted centroid
    // for all layers
...
const CartesianVector PfoCreationAlgorithm::GetEnergyWeightedCentroid(const Cluster* const pCluster,
...
StatusCode PfoCreationAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      XmlHelper::ReadValue(xmlHandle, "ShouldCreateTrackBasedPfos", m_shouldCreateTrackBasedPfos));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ShouldCreateNeutralPfos", m_shouldCreateNeutralPfos));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MinClusterElectromagneticEnergy", m_minClusterElectromagneticEnergy));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "AllowSingleLayerClusters", m_allowSingleLayerClusters));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "PhotonPositionAlgorithm", m_photonPositionAlgorithm));
