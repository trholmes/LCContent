...
PfoCreationParentAlgorithm::PfoCreationParentAlgorithm()
    : m_restoreOriginalClusterList(false), m_restoreOriginalTrackList(false), m_replaceCurrentPfoList(true),
...
StatusCode PfoCreationParentAlgorithm::Run() {
  ...
  if (!m_inputClusterListName.empty()) {
    ...
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                             PandoraContentApi::ReplaceCurrentList<Cluster>(*this, m_inputClusterListName));
  }
  ...
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           PandoraContentApi::RunDaughterAlgorithm(*this, m_pfoCreationAlgorithmName));
  ...
  if (!pPfoList->empty()) {
    ...
    if (m_replaceCurrentPfoList)
      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                               PandoraContentApi::ReplaceCurrentList<Pfo>(*this, m_pfoListName));
  }
  ...
  if (m_restoreOriginalClusterList && !m_inputClusterListName.empty()) {
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                             PandoraContentApi::ReplaceCurrentList<Cluster>(*this, originalClusterListName));
  }
...
StatusCode PfoCreationParentAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           XmlHelper::ProcessFirstAlgorithm(*this, xmlHandle, m_pfoCreationAlgorithmName));

  // Input parameters: names of input cluster/track lists and whether they should persist as current lists after
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      XmlHelper::ReadValue(xmlHandle, "RestoreOriginalClusterList", m_restoreOriginalClusterList));
  ...
  // Output parameters: name of output pfo and vertex lists and whether they should subsequently be used as the current
  // lists
