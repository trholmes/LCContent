...
StatusCode PfoPreparationAlgorithm::Run() {
  ...
  for (StringVector::const_iterator iter = m_candidateListNames.begin(), iterEnd = m_candidateListNames.end();
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_INITIALIZED, !=,
                                    PandoraContentApi::SaveList(*this, *iter, m_mergedCandidateListName, pfosToSave));
  }
  ...
  if (STATUS_CODE_SUCCESS != PandoraContentApi::ReplaceCurrentList<Pfo>(*this, m_mergedCandidateListName)) {
    std::cout << "PfoPreparationAlgorithm: empty pfo list for use by subsequent algorithms and client application."
              << std::endl;
