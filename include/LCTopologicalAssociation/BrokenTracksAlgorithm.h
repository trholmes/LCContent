...
private:
  ...
  public:
     ...
     */
    ClusterFitRelation(const pandora::Cluster* const pCluster, const pandora::ClusterFitResult& startFitResult,
  ...
  unsigned int
      m_minOccupiedLayersForStartFit; ///< Min number of occupied layers in cluster to allow fit to start of cluster
  unsigned int
      m_minOccupiedLayersForEndFit; ///< Min number of occupied layers in cluster to allow fit to end of cluster
  ...
  float m_trackMergePerpCutFine;   ///< Fine granularity cut on perp. distance between fit directions and centroid
                                   ///< difference
  float m_trackMergePerpCutCoarse; ///< Coarse granularity cut on perp. distance between fit directions and centroid
                                   ///< difference
...
inline BrokenTracksAlgorithm::ClusterFitRelation::ClusterFitRelation(const pandora::Cluster* const pCluster,
                                                                     const pandora::ClusterFitResult& startFitResult,
                                                                     const pandora::ClusterFitResult& endFitResult)
    : m_isDefunct(false), m_pCluster(pCluster), m_startFitResult(startFitResult), m_endFitResult(endFitResult) {}
...
inline void
BrokenTracksAlgorithm::ClusterFitRelation::SetStartFitResult(const pandora::ClusterFitResult& startFitResult) {
...
inline bool BrokenTracksAlgorithm::ClusterFitRelation::IsDefunct() const { return m_isDefunct; }

//------------------------------------------------------------------------------------------------------------------------------------------

inline void BrokenTracksAlgorithm::ClusterFitRelation::SetAsDefunct() { m_isDefunct = true; }
