...
private:
   ...
   */
  pandora::StatusCode GetClusterListAndNameMap(pandora::ClusterList& clusterList,
   ...
   */
  pandora::StatusCode PrepareClusters(const pandora::ClusterList& clusterList, pandora::ClusterVector& daughterVector,
                                      pandora::ClusterVector& parentVector) const;
   ...
   */
  pandora::StatusCode PreSelectClusters(const pandora::ClusterVector& daughterVector,
                                        const pandora::ClusterVector& parentVector,
   ...
   */
  pandora::StatusCode MergeClusters(const ClusterClusterMap& daughterBestParentMap,
   ...
   */
  pandora::StatusCode GetClusterListName(const pandora::Cluster* const pCluster,
                                         const ClusterListToNameMap& clusterListToNameMap, std::string& listName) const;
   ...
   */
  float GetHadEnergyWeightedDistance2ToLine(const pandora::Cluster* const pCluster,
                                            const pandora::ClusterFitResult& clusterFitResult) const;

  /**
   *  @brief  Get Hadronic energy weighted distance squared of all calo hits in a layer to a line of the the fit of 2nd
   * cluster
   *
   *  @param  pCluster address of the cluster
   *  @param  clusterFitResult 2nd cluster fit result
  ...
  float m_centroidDistance2Cut;   ///< Centroid distance squared cut for the distance between centroid of first layer of
                                  ///< daughter and last layer of parent
  float m_fullClosestApproachCut; ///< The cut for small closest approach for full fit of daughter and parent
  float
      m_contactClosestApproachCut; ///< The cut for small closest approach for fit using 2 layers of daughter and parent
  ...
  float m_daughterDistance2ToParentFitCut; ///< The cut for Hadronic energy weighted calo hit distance to the parent fit
                                           ///< result
