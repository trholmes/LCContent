...
protected:
   ...
   */
  virtual pandora::StatusCode PerformFragmentation(const pandora::Cluster* const pOriginalCluster,
                                                   const pandora::Track* const pTrack, unsigned int showerStartLayer,
   ...
   */
  virtual pandora::StatusCode MakeClusterFragments(const unsigned int showerStartLayer,
                                                   const unsigned int showerEndLayer,
                                                   const pandora::Cluster* const pOriginalCluster,
                                                   const pandora::Cluster*& pMipCluster,
                                                   const pandora::Cluster*& pPhotonCluster) const;
   ...
   */
  virtual pandora::StatusCode GetDistanceToTrack(const pandora::Cluster* const pCluster,
  ...
  unsigned int
      m_nLayersForShowerRegion;   ///< To identify shower region: number of layers with shower hit, but no mip hit
  ...
  unsigned int
      m_minShowerRegionSpan; ///< 1st pair of cuts: Minimum layers spanned by shower region to allow fragmentation
  unsigned int m_maxShowerStartLayer2; ///< 2nd pair of cuts: Maximum shower start layer to allow fragmentation
  unsigned int
      m_minShowerRegionSpan2; ///< 2nd pair of cuts: Minimum layers spanned by shower region to allow fragmentation
  ...
  float
      m_additionalPadWidthsCoarse; ///< Coarse granularity adjacent pad widths used to calculate cone approach distance
};
