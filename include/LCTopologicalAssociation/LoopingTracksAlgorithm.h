...
private:
   ...
   */
  float GetClosestDistanceBetweenOuterLayerHits(const pandora::Cluster* const pClusterI,
  ...
  float m_fitDirectionDotProductCutFine;   ///< Fine granularity cut on max value of dot product between cluster fit
                                           ///< directions
  float m_fitDirectionDotProductCutCoarse; ///< Coarse granularity cut on max value of dot product between cluster fit
                                           ///< directions

  float
      m_closestHitDistanceCutFine; ///< Fine granularity cut on distance between cluster hits in outermost pseudolayers
  float m_closestHitDistanceCutCoarse; ///< Coarse granularity cut on distance between cluster hits in outermost
                                       ///< pseudolayers

  float m_fitResultsClosestApproachCutFine;   ///< Fine granularity cut on closest distance of approach between two
                                              ///< cluster fits
  float m_fitResultsClosestApproachCutCoarse; ///< Coarse granularity cut on closest distance of approach between two
                                              ///< cluster fits

  unsigned int m_nGoodFeaturesForClusterMerge; ///< Number of identified "good features" required to merge fine
  ...
  unsigned int
      m_goodFeaturesMaxLayerDifference; ///< Max difference between cluster outer pseudo layers for good feature
  float m_goodFeaturesMinMipFraction;   ///< Min cluster mip fraction for good feature
};

//------------------------------------------------------------------------------------------------------------------------------------------

inline LoopingTracksAlgorithm::ClusterFitRelation::ClusterFitRelation(const pandora::Cluster* const pCluster,
                                                                      const pandora::ClusterFitResult& clusterFitResult)
    : m_isDefunct(false), m_pCluster(pCluster), m_clusterFitResult(clusterFitResult) {}
...
inline void
LoopingTracksAlgorithm::ClusterFitRelation::SetClusterFitResult(const pandora::ClusterFitResult& clusterFitResult) {
  m_clusterFitResult = clusterFitResult;
}

//------------------------------------------------------------------------------------------------------------------------------------------

inline bool LoopingTracksAlgorithm::ClusterFitRelation::IsDefunct() const { return m_isDefunct; }

//------------------------------------------------------------------------------------------------------------------------------------------

inline void LoopingTracksAlgorithm::ClusterFitRelation::SetAsDefunct() { m_isDefunct = true; }
