...
private:
  ...
  /**
   *  @brief  Run the reclustering processes to compare the output of n clustering algorithms with the default saved
   * list
   */
   ...
   */
  void CompareClusters(const pandora::ClusterList& clusterList1, const pandora::ClusterList& clusterList2,
                       pandora::ClusterList& outputList1, pandora::ClusterList& outputList2) const;
   ...
   */
  void PopulateMaps(const pandora::ClusterList& clusterList, ClusterToHitListMap& clusterToHitListMap,
  ...
  bool m_restoreOriginalCaloHitList;  ///< Whether to restore the original calo hit list as the "current" list upon
                                      ///< completion

  std::string
      m_inputTrackListName; ///< The name of the input track list, containing the tracks for use in seeding clusters
  ...
  pandora::StringVector m_comparisonAlgorithms; ///< The ordered list of reclustering algorithms to be used, output then
