...
private:
   ...
   *  @param  clusterVector to receive the addresses of clusters created (which could also be accessed via current
   * cluster list)
   */
  pandora::StatusCode SeedClustersWithTracks(const pandora::TrackList* const pTrackList,
  ...
  /**
   *  @brief  Update the properties of the current clusters, calculating their current directions and storing the fit
   * results in a provided map
   *
   ...
   */
  pandora::StatusCode GetCurrentClusterFitResults(const pandora::ClusterVector& clusterVector,
  ...
  /**
   *  @brief  Get the "generic distance" between a calo hit and a cluster; the smaller the distance, the stronger the
   * association
   *
   ...
   */
  pandora::StatusCode GetGenericDistanceToHit(const pandora::Cluster* const pCluster,
                                              const pandora::CaloHit* const pCaloHit, const unsigned int searchLayer,
                                              const ClusterFitResultMap& clusterFitResultMap,
   ...
   */
  pandora::StatusCode GetDistanceToHitInSameLayer(const pandora::CaloHit* const pCaloHit,
                                                  const pandora::CaloHitList* const pCaloHitList,
                                                  float& distance) const;

  /**
   *  @brief  Get the smallest cone approach distance between a calo hit and all the hits in a cluster, using a
   * specified measurement of the cluster direction
   *
   *  @param  pCaloHit address of the calo hit
   *  @param  pCaloHitList
   ...
   */
  pandora::StatusCode GetConeApproachDistanceToHit(const pandora::CaloHit* const pCaloHit,
                                                   const pandora::CaloHitList* const pCaloHitList,
   ...
   */
  pandora::StatusCode GetConeApproachDistanceToHit(const pandora::CaloHit* const pCaloHit,
                                                   const pandora::CartesianVector& clusterPosition,
                                                   const pandora::CartesianVector& clusterDirection,
                                                   float& distance) const;

  /**
   *  @brief  Get the distance between a calo hit and the track seed (projected) position at the calorimeter surface,
   * ONLY calculated if: 1) the calo hit is within the first m_maxLayersToTrackSeed OR 2) cluster contains a hit
   * consistent with the track seed within the last m_maxLayersToTrackLikeHit
   *
   *  @param  pCluster address of the cluster
   *  @param  pCaloHit address of the calo hit
   ...
   */
  pandora::StatusCode GetDistanceToTrackSeed(const pandora::Cluster* const pCluster,
                                             const pandora::CaloHit* const pCaloHit, unsigned int searchLayer,
                                             float& distance) const;
  ...
  unsigned int
      m_layersToStepBackCoarse; ///< Number of layers to step back when associating a coarse granularity hit to clusters
  ...
  float
      m_maxClusterDirProjection; ///< Max projection of cluster-hit separation in cluster dir to calculate cone distance
  float
      m_minClusterDirProjection; ///< Min projection of cluster-hit separation in cluster dir to calculate cone distance
  ...
  unsigned int
      m_maxLayersToTrackSeed; ///< Max number of layers to the track seed for trackSeedDistance to be calculated
  unsigned int
      m_maxLayersToTrackLikeHit; ///< Max number of layers to a "track-like" hit for trackSeedDistance to be calculated

  unsigned int
      m_nLayersSpannedForFit; ///< Min number of layers spanned by cluster before current direction fit is performed
  unsigned int m_nLayersSpannedForApproxFit; ///< If current fit fails, but cluster spans enough layers, calculate an
                                             ///< approximate fit
