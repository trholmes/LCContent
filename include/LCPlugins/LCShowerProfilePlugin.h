...
public:
  ...
  void CalculateLongitudinalProfile(const pandora::Cluster* const pCluster, float& profileStart,
                                    float& profileDiscrepancy) const;
  void CalculateTransverseProfile(const pandora::Cluster* const pCluster, const unsigned int maxPseudoLayer,
                                  ShowerPeakList& showerPeakList) const;
  void CalculateTransverseProfile(const pandora::Cluster* const pCluster, const unsigned int maxPseudoLayer,
                                  ShowerPeakList& showerPeakList, const bool inclusiveMode) const;
  void CalculateTrackBasedTransverseProfile(const pandora::Cluster* const pCluster, const unsigned int maxPseudoLayer,
...
private:
  ...
  public:
    ...
    pandora::CaloHitList
        m_unusedCaloHitList; ///< The list of calo hits unused for shower peak finding, needed for inclusive mode
  };
  ...
  /**
   *  @brief  Calculate transverse shower peak objects for a cluster and get the list of peaks identified in the
   * profile, for clusters without tracks
   *
   ...
   */
  void CalculateTracklessTransverseShowers(const pandora::Cluster* const pCluster, const unsigned int maxPseudoLayer,
                                           TwoDShowerProfile& showerProfile,
                                           ShowerPeakObjectVector& showerPeakObjectVector) const;

  /**
   *  @brief  Calculate transverse shower peak objects for a cluster and get the list of peaks identified in the
   * profile, for clusters close to tracks
   *
   *  @param  pCluster the address of the cluster
   *  @param  maxPseudoLayer the maximum pseudo layer to consider
   ...
   */
  void CalculateTrackNearbyTransverseShowers(const pandora::Cluster* const pCluster, const unsigned int maxPseudoLayer,
                                             const pandora::Track* const pMinTrack,
                                             const pandora::TrackVector& trackVector, TwoDShowerProfile& showerProfile,
   ...
   *  @param  showerProfile two dimensional shower profile to consider
   */
  void InitialiseTransverseProfile(const pandora::Cluster* const pCluster, const unsigned int maxPseudoLayer,
   ...
   *  @param  trackProjectionVector the 2D positions of track projections to receive and speed up subsequent calculation
   */
  void InitialiseTransverseProfileWithTracks(const pandora::Cluster* const pCluster, const unsigned int maxPseudoLayer,
                                             const pandora::Track* const pMinTrack,
                                             const pandora::TrackVector& trackVector, TwoDShowerProfile& showerProfile,
                                             TwoDBinVector& trackProjectionVector) const;
   ...
   *  @param  pMinTrack the closest track to the shower profile to speed up calculation
   */
  void CalculateProjectionAxes(const pandora::Cluster* const pCluster, pandora::CartesianVector& innerLayerCentroid,
                               pandora::CartesianVector& uAxis, pandora::CartesianVector& vAxis,
                               const pandora::Track* const pMinTrack = NULL) const;
   ...
   *  @param  showerProfile two dimensional shower profile to receive
   */
  void InitialiseTwoDShowerProfile(const pandora::Cluster* const pCluster, const unsigned int maxPseudoLayer,
                                   const pandora::CartesianVector& innerLayerCentroid,
                                   const pandora::CartesianVector& uAxis, const pandora::CartesianVector& vAxis,
                                   TwoDShowerProfile& showerProfile) const;
   ...
   *  @param  trackProjectionVector the 2D positions of track projections to receive and speed up subsequent calculation
   */
  void FindTracksProjection(const pandora::Cluster* const pCluster, const pandora::TrackVector& trackVector,
                            const pandora::CartesianVector& innerLayerCentroid, const pandora::CartesianVector& uAxis,
                            const pandora::CartesianVector& vAxis, TwoDBinVector& trackProjectionVector) const;
   ...
   */
  void FindRawPeaksInTwoDShowerProfile(TwoDShowerProfile& showerProfile,
                                       ShowerPeakObjectVector& showerPeakObjectVector) const;
   ...
   *  @param  showerPeakObjectVector the 2D peak object to receive
   */
  void AssociateUnavailableBinsToPeaks(const TwoDShowerProfile& showerProfile,
                                       ShowerPeakObjectVector& showerPeakObjectVector) const;
   ...
   *  @param  showerPeakObjectVector the 2D peak object to receive
   */
  void AssociateBinsToPeaks(const TwoDShowerProfile& showerProfile,
                            ShowerPeakObjectVector& showerPeakObjectVector) const;
   ...
   */
  void MarkPeaksCloseToTracks(const TwoDBinVector& trackProjectionVector,
                              ShowerPeakObjectVector& showerPeakObjectVector) const;
   ...
   */
  void MatchPeaksInTwoSlices(const ShowerPeakObjectVector& showerPeakObjectVectorFirst,
   ...
   */
  void ConvertBinsToShowerLists(const TwoDShowerProfile& showerProfile,
                                const ShowerPeakObjectVector& showerPeakObjectVector,
                                ShowerPeakList& showerPeakListPhoton, ShowerPeakList& showerPeakListNonPhoton,
   ...
   */
  void FindHitPositionProjection(const pandora::CartesianVector& hitPosition,
                                 const pandora::CartesianVector& innerLayerCentroid,
                                 const pandora::CartesianVector& uAxis, const pandora::CartesianVector& vAxis,
                                 const int nOffsetBins, const float cellLengthScale, int& uBin, int& vBin) const;
   ...
   */
  void FindBoundaryBins(const int uBin, const int vBin, const int uEdgeLow, const int uEdgeHigh, const int vEdgeLow,
                        const int vEdgeHigh, int& uEdgeBin, int& vEdgeBin) const;
   ...
   */
  void CalculateBestPeakUsingMetric(ShowerPeakObjectVector& showerPeakObjectVector, const int uBin, const int vBin,
  ...
  float m_transProfileMinDisTrackMatch; ///< The maximum allowed shift of 2D distance of the peak position through the
                                        ///< slices
};

//------------------------------------------------------------------------------------------------------------------------------------------

inline void LCShowerProfilePlugin::CalculateTransverseProfile(const pandora::Cluster* const pCluster,
...
inline LCShowerProfilePlugin::ShowerProfileEntry::ShowerProfileEntry()
    : m_isAvailable(true), m_energy(0.f), m_potentialPeak(true) {}

//------------------------------------------------------------------------------------------------------------------------------------------

inline LCShowerProfilePlugin::ShowerPeakObject::ShowerPeakObject(const float energy, const int uBin, const int vBin)
    : m_isAvailable(true), m_isPhotonCandidate(true), m_energy(energy), m_uBin(uBin), m_vBin(vBin) {
  m_associatedBins.push_back(std::make_pair(uBin, vBin));
}

//------------------------------------------------------------------------------------------------------------------------------------------

inline float LCShowerProfilePlugin::ShowerPeakObject::GetPeakEnergy() const { return m_energy; }

//------------------------------------------------------------------------------------------------------------------------------------------

inline int LCShowerProfilePlugin::ShowerPeakObject::GetPeakUBin() const { return m_uBin; }

//------------------------------------------------------------------------------------------------------------------------------------------

inline int LCShowerProfilePlugin::ShowerPeakObject::GetPeakVBin() const { return m_vBin; }
