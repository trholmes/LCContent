...
private:
  ...
  /**
   *  @brief  Extract energy weighted mean times from the hits in a cluster. Separate times are also extracted for ecal
   * and hcal endcap regions, alongside the number of hits in these regions.
   *
   ...
   */
  void GetClusterTimes(const pandora::Cluster* const pCluster, float& meanTime, float& meanTimeECal,
                       unsigned int& nECalHits, float& meanHCalEndCapTime, unsigned int& nHCalEndCapHits) const;
  ...
  float m_neutralHadronBarrelPtCutForLooseTiming; ///< pt above which loose timing cuts are applied to neutral hadrons
                                                  ///< in barrel
