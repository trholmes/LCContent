...
private:
   ...
   *  @brief  pdf variables
   */
  enum PDFVar { PEAKRMS, RMSXYRATIO, LONGPROFILESTART, LONGPROFILEDISCREPANCY, PEAKENERGYFRACTION, MINDISTANCETOTRACK };
  ...
  /**
   *  @brief  Get individual showers(clusters) from the big cluster, for the cluster far from charged tracks projection.
   * Main power horse
   *
   *  @param  pCluster address of the cluster
   *  @param  showersPhoton shower peak list of photon candidates from the big cluster
   */
  pandora::StatusCode GetTracklessClusterShowerList(const pandora::Cluster* const pCluster,
                                                    pandora::ShowerProfilePlugin::ShowerPeakList& showersPhoton) const;

  /**
   *  @brief  Get individual showers(clusters) from the big cluster, for the cluster close to charged tracks projection.
   * Main power horse
   *
   *  @param  pCluster address of the cluster
   ...
   */
  pandora::StatusCode GetTrackClusterShowerList(const pandora::Cluster* const pCluster,
                                                const pandora::Track* const pMinTrack,
                                                const pandora::TrackVector trackVector,
                                                pandora::ShowerProfilePlugin::ShowerPeakList& showersPhoton,
                                                pandora::ShowerProfilePlugin::ShowerPeakList& showersCharged) const;
   ...
   */
  pandora::StatusCode CreatePhotons(const pandora::Cluster* const pCluster,
                                    const pandora::ShowerProfilePlugin::ShowerPeakList& showersPhoton,
   ...
   */
  pandora::StatusCode InitialiseFragmentation(const pandora::Cluster* const pCluster,
   ...
   *  @param  peakClusterListName new cluster list name
   */
  pandora::StatusCode EndFragmentation(const bool usedCluster, const std::string& originalClusterListName,
   ...
   */
  pandora::StatusCode CreateClustersAndSetPhotonID(const pandora::ShowerProfilePlugin::ShowerPeakList& showersPhoton,
   ...
   */
  pandora::StatusCode CreateCluster(const pandora::ShowerProfilePlugin::ShowerPeak& showerPeak,
                                    const pandora::Cluster*& pPeakCluster) const;
   ...
   *  @param  isFromTrack true for the cluster close to a track
   */
  pandora::StatusCode CheckAndSetPhotonID(const pandora::ShowerProfilePlugin::ShowerPeak& showerPeak,
                                          const pandora::Cluster* const pPeakCluster, const float wholeClusuterEnergy,
                                          bool& isPhoton, const bool isFromTrack) const;
   ...
   */
  pandora::StatusCode CalculateForPhotonID(const pandora::ShowerProfilePlugin::ShowerPeak& showerPeak,
                                           const pandora::Cluster* const pPeakCluster, const float wholeClusuterEnergy,
   ...
   *  @param  isFromTrack true for the cluster close to a track
   */
  bool IsPhoton(const pandora::Cluster* const pPeakCluster, const PDFVarFloatMap& pdfVarFloatMap,
                const bool isFromTrack) const;
   ...
   */
  pandora::StatusCode GetMinDistanceToTrack(const pandora::Cluster* const pCluster,
                                            const pandora::TrackVector& trackVector, float& minDistance,
   ...
   */
  pandora::StatusCode GetNEnergyBins(const pandora::TiXmlHandle xmlHandle, const std::string& nEnergyBinsStr,
   ...
   */
  pandora::StatusCode GetEnergyBinLowerEdges(const pandora::TiXmlHandle xmlHandle,
   ...
   */
  pandora::StatusCode GetNSignalBackgroundEvts(const pandora::TiXmlHandle xmlHandle,
   ...
   */
  pandora::StatusCode FillPDFVarLikelihoodPDFMapParameters(const pandora::TiXmlHandle xmlHandle, const PDFVar histVar,
                                                           const std::string& nBinStr, const int nBinDefault,
                                                           const std::string& lowValueStr, const float lowValueDefault,
                                                           const std::string& highValueStr,
   ...
   *  @param  showersPhoton shower peak list for photon candidate
   */
  pandora::StatusCode CreatePhotonsForTraining(const pandora::Cluster* const pCluster,
                                               const pandora::ShowerProfilePlugin::ShowerPeakList& showersPhoton);
   ...
   */
  pandora::StatusCode CreateClustersAndTrainPhotonID(const pandora::ShowerProfilePlugin::ShowerPeakList& showersPhoton,
   ...
   *  @param  wholeClusuterEnergy the energy of the whole cluster
   */
  pandora::StatusCode TrainPhotonID(const pandora::ShowerProfilePlugin::ShowerPeak& showerPeak,
                                    const pandora::Cluster* const pCluster, const float wholeClusuterEnergy);
  ...
  bool m_transProfileEcalOnly;         ///< Transverse profile shower calculator uses EcalOnly. Can be overridden by the
...
inline PhotonReconstructionAlgorithm::LikelihoodPDFObject::LikelihoodPDFObject(const std::string& pdfVarName)
    : m_pdfVarName(pdfVarName), m_nBins(0), m_lowValue(0.f), m_highValue(0.f), m_pSignalPDF(NULL),
