...
public:
   ...
   *  @param  distance
   */
  void findNearestNeighbour(const KDTreeNodeInfoT<DATA, DIM>& point, const KDTreeNodeInfoT<DATA, DIM>*& result,
                            float& distance);
...
private:
   ...
   *  @param  best_dist
   */
  void recNearestNeighbour(unsigned depth, const KDTreeNodeT<DATA, DIM>* current,
                           const KDTreeNodeInfoT<DATA, DIM>& point, const KDTreeNodeT<DATA, DIM>*& best_match,
                           float& best_dist);
...
inline KDTreeLinkerAlgo<DATA, DIM>::KDTreeLinkerAlgo()
    : root_(nullptr), nodePool_(nullptr), nodePoolSize_(-1), nodePoolPos_(-1), closestNeighbour(nullptr),
...
template <typename DATA, unsigned DIM>
inline void KDTreeLinkerAlgo<DATA, DIM>::findNearestNeighbour(const KDTreeNodeInfoT<DATA, DIM>& point,
...
inline void KDTreeLinkerAlgo<DATA, DIM>::recNearestNeighbour(unsigned int depth, const KDTreeNodeT<DATA, DIM>* current,
  ...
  if (current->left == nullptr && current->right == nullptr) {
    ...
    // If we're here we're returned so best_dist is filled. Compare to this node and see if it's a better match. If it
    // is, update result
...
template <typename DATA, unsigned DIM>
inline KDTreeNodeT<DATA, DIM>* KDTreeLinkerAlgo<DATA, DIM>::recBuild(int low, int high, int depth,
                                                                     const KDTreeBoxT<DIM>& region) {
