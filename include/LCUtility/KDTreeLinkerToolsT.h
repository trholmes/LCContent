...
/**
 *  @brief  Box structure used to define 2D field. It's used in KDTree building step to divide the detector space (ECAL,
 * HCAL...) and in searching step to create a bounding box around the demanded point (Track collision point, PS
 * projection...).
 */
 ...
 */
template <typename T>
KDTreeCube fill_and_bound_3d_kd_tree_by_index(const std::vector<const T*>& points,
 ...
 */
KDTreeTesseract fill_and_bound_4d_kd_tree(const pandora::Algorithm* const caller,
 ...
 *  @return KDTreeCube
 */
KDTreeCube build_3d_kd_search_region(const pandora::CaloHit* const point, const float x_span, const float y_span,
                                     const float z_span);
 ...
 *  @return KDTreeTesseract
 */
KDTreeTesseract build_4d_kd_search_region(const pandora::CaloHit* const point, const float x_span, const float y_span,
                                          const float z_span, const float search_layer);
 ...
 *  @return KDTreeTesseract
 */
KDTreeTesseract build_4d_kd_search_region(const pandora::CartesianVector& pos, const float x_span, const float y_span,
                                          const float z_span, const float search_layer);
...
kdtree_type_adaptor<const pandora::Track>::position(const pandora::Track* const t) {
  return t->GetTrackStateAtCalorimeter().GetPosition();
}

template <>
inline const pandora::CartesianVector&
kdtree_type_adaptor<const pandora::CaloHit>::position(const pandora::CaloHit* const t) {
  return t->GetPositionVector();
}

template <>
inline const pandora::CartesianVector&
kdtree_type_adaptor<const pandora::CartesianVector>::position(const pandora::CartesianVector* const t) {
  return *t;
}
